...
## TASK 8H-1 - Explicit One-Shot Live-Execution Authorization Gate (2026-07-10)

- **Status:** `EXPLICIT_ONE_SHOT_LIVE_AUTHORIZATION_GATE_SOURCE_PRESENT_RECORDING_ONLY_TESTED_PRODUCTION_PROVIDER_NOT_CONSTRUCTED_NO_NATIVE_EXECUTION_NO_BINDING_NO_MUTATION_LIVE_EXECUTION_STILL_UNAUTHORIZED_PENDING_INDEPENDENT_AUDIT`.
- **Evidence:** `docs/evidence/live-execution-authorization-gate-task-8h-1.json`, schema `chatpad-live-execution-authorization-gate-evidence-v1`, SHA-256 `2EAC8D3C707C53F77859620DC9D7D43903DC61F334774898BBFD56944977DBF2`.
- **Gate source:** `tools/ExactInstance/ChatpadLiveExecutionAuthorizationGate.psm1`; managed helper `tools/ExactInstance/ChatpadLiveAuthorizationRegistry.cs`.
- **Public surface:** one exported function, `New-ChatpadOneShotLiveNativeApplyAuthorization`. It returns a structured fail-closed result and does not expose a production execution command.
- **Required inputs:** exact `APPLY` operation; exact ordered target chain; shared ContainerId `{828F4587-006F-5AD1-B169-6AF57905DFDE}`; operator confirmation `operator-confirmation-c445630fbb303c7c`; phrase `AUTHORIZE_ONE_LIVE_NATIVE_APPLY_ATTEMPT`; accepted TASK 8E/8F/8G evidence identities; audited TASK 8G commit `79ec174dabd7e73f2d01021168921021bc118702`; current critical source hashes; rollback/recovery review acknowledgement; one-shot attempt acknowledgement.
- **TASK 8H-1R1 remediation:** caller-supplied critical-source hashes must match the fixed accepted inventory, the gate hashes the current fixed source inventory at runtime, single-item array and string acknowledgement values are rejected, and source-integrity fixture tampering, missing files, directories, and path traversal are rejected. The gate source itself and managed helper remain externally validated root-of-trust files.
- **One-shot mechanism:** a separate `Chatpad.LiveAuthorization.LiveNativeApplyAuthorization` object is registered in a managed `ConditionalWeakTable` and consumed with `Interlocked.CompareExchange`. It is process-bound, reference-identity-bound, fingerprint-bound, non-serializable as usable authority, and has no reset/retry/provider-binding method.
- **Authority separation:** TASK 8G recording authorization is not accepted by the live gate consumer, and the TASK 8H live authorization is not accepted by the existing recording-only coordinator path.
- **Offline tests:** TASK 8H passed in PowerShell 7.6.3 and Windows PowerShell 5.1.26100.8655 with 75 tests / 2146 assertions in each runtime. The bounded actual-registry race ran 16 iterations with two contenders, one successful consumption, one replay rejection, zero production constructions, and zero native invocations.
