...
All event sites use this safety baseline unless a row narrows it further:
emit no event while holding the request-owner spinlock, after the owning
context lifetime ends, after the relevant object was deleted, or from a path
where WPP emission would be unsafe. Current implementation-time IRQL is not yet
compiled or independently proven, so every event row uses the conservative
phrase required by this design.

| ID | Name | Level | Keyword | Emitting function | Exact emission point | Required fields | Semantics | Expected IRQL | Maximum frequency | First-load criterion | Failure or rollback action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1000 | DRIVER_ENTRY_STARTED | Info | DriverEntry | `DriverEntry` | first statement | none beyond common | driver entry reached | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per driver load | required success evidence; mandatory in conclusive success | no rollback; continue normal driver initialization |
| 1001 | WDF_DRIVER_CREATE_ATTEMPTED | Info | DriverEntry | `DriverEntry` | immediately before `WdfDriverCreate` | none | framework driver creation attempted | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per driver load | required success evidence; mandatory in conclusive success | no rollback; continue to framework driver creation |
| 1002 | WDF_DRIVER_CREATE_FAILED | Error | DriverEntry | `DriverEntry` | failure branch | returned `NTSTATUS` | terminal driver-entry failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per driver load | required failure evidence; proves framework driver creation failure | return recorded failure status; no device rollback exists |
| 1003 | DRIVER_ENTRY_COMPLETED | Info | DriverEntry | `DriverEntry` | before return | returned `NTSTATUS` | driver entry completed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per driver load | terminal evidence for driver entry; mandatory in conclusive success | no rollback; return recorded driver-entry status |
| 1100 | DEVICE_ADD_ENTERED | Info | DeviceAdd | `ChatpadEvtDeviceAdd` | first executable statement | attempt ID | device-add reached | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add attempt | required success evidence; mandatory in conclusive success | no rollback; allocate/use attempt correlation and continue |
| 1101 | DEVICE_CREATE_ATTEMPTED | Info | DeviceAdd | `ChatpadEvtDeviceAdd` | before `WdfDeviceCreate` | attempt ID | device/context creation attempted | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add attempt | required success evidence; mandatory in conclusive success | no rollback; continue to WDF device creation |
| 1102 | DEVICE_CREATE_FAILED | Error | DeviceAdd | `ChatpadEvtDeviceAdd` | failure branch | `NTSTATUS` | no device context created | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per device-add attempt | required failure evidence; proves device creation failure | return recorded failure status; no device-context rollback exists |
| 1103 | DEVICE_CONTEXT_INITIALIZING | Info | DeviceAdd | `ChatpadEvtDeviceAdd` | after `WdfDeviceCreate`, before scalar context setup | context version constants only | context initialization began | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful device creation | allowed progress evidence | no rollback; continue context initialization |
| 1104 | DEVICE_CONTEXT_READY | Info | DeviceAdd | `ChatpadEvtDeviceAdd` | after scalar context setup | signature/version status | context initialized | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful device creation | allowed progress evidence | no rollback; continue owner initialization |
| 1105 | DEVICE_CREATE_COMPLETED | Info | DeviceAdd | `ChatpadEvtDeviceAdd` | after context stored | `NTSTATUS=STATUS_SUCCESS` | device/context creation succeeded | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful device creation | required success evidence; mandatory in conclusive success | no rollback; WDF context ownership is available |
| 1200 | OWNER_STORAGE_INIT_STARTED | Info | Owner | `ChatpadEvtDeviceAdd` | before initializer call | object-presence snapshot | owner init started | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add attempt after device creation | allowed progress evidence | no rollback; continue owner storage initialization |
| 1201 | OWNER_STORAGE_INIT_COMPLETED | Info | Owner | `ChatpadEvtDeviceAdd` | after initializer success | storage result | owner init completed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful owner initialization | required success evidence; mandatory in conclusive success | no rollback; continue pre-object validation |
| 1202 | OWNER_STORAGE_INIT_FAILED | Error | Owner | `ChatpadEvtDeviceAdd` | after initializer failure | storage result, mapped status | terminal owner init failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per device-add attempt | required failure evidence; proves owner storage initialization failure | return mapped status; require device-context destruction by WDF |
| 1203 | PRE_OBJECT_VALIDATION_STARTED | Info | Owner | `ChatpadEvtDeviceAdd` | before prevalidation | snapshot | validation started | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per owner-initialized attempt | allowed progress evidence | no rollback; continue pre-object validation |
| 1204 | PRE_OBJECT_VALIDATION_FAILED | Error | Owner | `ChatpadEvtDeviceAdd` | failure branch | validation result, invariant mask class | terminal prevalidation failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per device-add attempt | required failure evidence; proves invalid pre-object state | return failure status; require device-context destruction by WDF |
| 1205 | PRE_OBJECT_VALIDATION_COMPLETED | Info | Owner | `ChatpadEvtDeviceAdd` | after success | validation result | prevalidation passed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful prevalidation | required success evidence; mandatory in conclusive success | no rollback; continue orchestration |
| 1300 | ORCHESTRATION_STARTED | Info | Orchestration | `ChatpadEvtDeviceAdd` | before orchestrator call | pre-orchestration snapshot | production orchestration began | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add orchestration attempt | required success evidence; mandatory in conclusive success | no rollback; continue orchestration |
| 1301 | ORCHESTRATION_STAGE_ENTERED | Verbose | Orchestration | orchestrator | each stage boundary | stage enum | bounded stage progress | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per finite orchestration stage, maximum 9 per attempt | allowed progress evidence | no operator action unless followed by failure or missing terminal event |
| 1302 | ORCHESTRATION_STAGE_COMPLETED | Verbose | Orchestration | orchestrator | each stage success | stage enum, snapshot | bounded stage success | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per finite orchestration stage, maximum 9 per attempt | allowed progress evidence | no operator action unless followed by failure or missing terminal event |
| 1303 | ORCHESTRATION_RETURNED | Info | Orchestration | `ChatpadEvtDeviceAdd` | immediately after call | function result, report result | orchestrator returned | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | required success evidence; mandatory in conclusive success, or required failure evidence | interpret result/report and continue to equality/status handling |
| 1304 | ORCHESTRATION_FUNCTION_RESULT | Info/Error | Orchestration | `ChatpadEvtDeviceAdd` | after return | function result enum | function result captured | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | diagnostic-only supporting evidence | compare with report result; failure value continues to mapped failure path |
| 1305 | ORCHESTRATION_REPORT_RESULT | Info/Error | Orchestration | `ChatpadEvtDeviceAdd` | after report validation | report result enum | report result captured | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | diagnostic-only supporting evidence | compare with function result; mismatch is terminal |
| 1306 | ORCHESTRATION_FUNCTION_REPORT_MATCHED | Info | Orchestration | `ChatpadEvtDeviceAdd` | equality branch | result enum | equality confirmed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | required success evidence; mandatory in conclusive success | no rollback if result is success; otherwise continue mapped failure handling |
| 1307 | ORCHESTRATION_FUNCTION_REPORT_MISMATCH | Error | Orchestration | `ChatpadEvtDeviceAdd` | mismatch branch | both result enums | terminal mismatch | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per orchestration attempt | required failure evidence; proves function/report mismatch | return `STATUS_INVALID_DEVICE_STATE`; immediate stop and no further runtime attempt |
| 1308 | ORCHESTRATION_REPORT_SUMMARY | Info/Error | Orchestration | `ChatpadEvtDeviceAdd` | before status mapping | bounded report schema | terminal orchestration summary | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | required success evidence in conclusive success and required failure evidence on orchestration failure | use summary to classify result; failed summaries require stop or rollback evidence |
| 1309 | ORCHESTRATION_STATUS_MAPPED | Info/Error | Orchestration | `ChatpadEvtDeviceAdd` | after mapping | result enum, status class | mapped `NTSTATUS` | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | required failure evidence when orchestration fails; diagnostic-only on success | return mapped failure status or continue on success |
| 1310 | ORCHESTRATION_UNEXPECTED_ENUM | Error | Orchestration | mapping/report helpers | default/unrecognized path | enum family, value class | taxonomy defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per unexpected enum family per attempt | invariant violation; presence fails first-load interpretation | return `STATUS_INVALID_DEVICE_STATE`; mark trace failed |
| 1311 | ORCHESTRATION_READY_PUBLISHED | Info | Orchestration | orchestrator | after ready publish | ready flags | owner-ready attempted | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per orchestration attempt | required success evidence; mandatory in conclusive success | no rollback; continue ready validation |
| 1312 | ORCHESTRATION_TERMINAL_CATEGORY | Info/Error | Orchestration | `ChatpadEvtDeviceAdd` | after report summary | terminal category | audit-friendly classification | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration attempt | terminal evidence for orchestration classification | follow category: continue success path, return failure, or require rollback evidence |
| 1400 | READY_VALIDATION_STARTED | Info | Readiness | `ChatpadEvtDeviceAdd` | before structural-ready validation | snapshot | structural check started | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per orchestration-success attempt | allowed progress evidence | no rollback; continue structural-ready validation |
| 1401 | READY_VALIDATION_PASSED | Info | Readiness | `ChatpadEvtDeviceAdd` | after success | snapshot | structural ready passed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful readiness validation | required success evidence; mandatory in conclusive success | no rollback; continue lifecycle initialization |
| 1402 | READY_VALIDATION_FAILED | Error | Readiness | `ChatpadEvtDeviceAdd` | failure branch | missing-object class, status | terminal structural failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per readiness validation attempt | required failure evidence; proves structural-ready failure | return failure status; require device-context destruction and no second load attempt |
| 1403 | READY_VALIDATION_MISSING_OBJECT | Error | Readiness | validation helper | specific missing flag | object flags | object graph incomplete | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per missing-object classification per attempt | required failure evidence; proves object graph incomplete | return failure status through readiness failure path |
| 1404 | READY_VALIDATION_INVARIANT_FAILED | Error | Readiness | validation helper | invariant failure | validation result class | structural invariant failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per readiness validation attempt | invariant violation; presence fails first-load interpretation | return `STATUS_INVALID_DEVICE_STATE`; mark trace failed |
| 1500 | LIFECYCLE_INIT_STARTED | Info | Lifecycle | `ChatpadEvtDeviceAdd` | before lifecycle init | none | lifecycle reached | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per readiness-passed attempt | required success evidence; mandatory in conclusive success | no rollback; continue lifecycle initialization |
| 1501 | LIFECYCLE_INIT_PASSED | Info | Lifecycle | `ChatpadEvtDeviceAdd` | after success | lifecycle result | lifecycle initialized | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful lifecycle initialization | required success evidence; mandatory in conclusive success | no rollback; continue mark-device-created |
| 1502 | LIFECYCLE_INIT_FAILED | Error | Lifecycle | `ChatpadEvtDeviceAdd` | after failure | lifecycle result, status | terminal lifecycle failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per lifecycle initialization attempt | required failure evidence; proves lifecycle initialization failure | return mapped status; require Level 2 device rollback during future runtime work |
| 1503 | MARK_DEVICE_CREATED_STARTED | Info | Lifecycle | `ChatpadEvtDeviceAdd` | before mark call | lifecycle snapshot | mark started | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per lifecycle-initialized attempt | allowed progress evidence; mandatory before conclusive mark pass | no rollback; continue mark-device-created |
| 1504 | MARK_DEVICE_CREATED_PASSED | Info | Lifecycle | `ChatpadEvtDeviceAdd` | after success | lifecycle snapshot | device-created mark passed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful mark | required success evidence; mandatory in conclusive success | no rollback; continue terminal success |
| 1505 | MARK_DEVICE_CREATED_FAILED | Error | Lifecycle | `ChatpadEvtDeviceAdd` | after failure | lifecycle result, snapshot | terminal mark failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per mark attempt | required failure evidence; proves device-created mark failure | return mapped status; require Level 2 device rollback during future runtime work |
| 1600 | ROLLBACK_STARTED | Warning | Cleanup | orchestrator | before rollback helper | reason, object snapshot | partial rollback began | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per rollback | required rollback evidence | continue rollback; collect before/after snapshots |
| 1601 | ROLLBACK_REASON | Warning | Cleanup | orchestrator | rollback start | terminal category | rollback reason | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per rollback | required rollback evidence | no separate control-flow change; explains rollback cause |
| 1602 | ROLLBACK_OBJECT_SNAPSHOT_BEFORE | Info | Cleanup | rollback helper | before deletions | object flags | pre-delete snapshot | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per rollback | required rollback evidence | continue rollback; do not emit after deletion |
| 1603 | ROLLBACK_COMPLETED | Info/Error | Cleanup | rollback helper | after rollback helper | rollback result, effects | rollback result | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per rollback | required rollback evidence | successful rollback continues terminal failure path; failed rollback escalates |
| 1604 | ROLLBACK_OBJECT_SNAPSHOT_AFTER | Info | Cleanup | rollback helper | after state update | object flags | post-delete state | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per rollback | required rollback evidence | no operator action unless snapshot is inconsistent |
| 1605 | DEVICE_CONTEXT_CLEANUP_ENTERED | Info | Cleanup | future cleanup callback | callback entry | attempt ID, object flags | device cleanup reached | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per cleanup callback | required cleanup evidence | no new ownership change; snapshot before context lifetime ends |
| 1606 | DEVICE_CONTEXT_CLEANUP_SNAPSHOT | Info | Cleanup | future cleanup callback | after snapshot | object flags, counters | cleanup state | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per cleanup callback | required cleanup evidence | mark trace inconclusive if required cleanup state is missing or inconsistent |
| 1607 | DEVICE_CONTEXT_CLEANUP_COMPLETED | Info | Cleanup | future cleanup callback | callback exit | final object flags | cleanup completed | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per cleanup callback | required cleanup evidence | no operator action unless followed by missing terminal evidence |
| 1608 | CLEANUP_INVARIANT_VIOLATION | Error | Cleanup | rollback/cleanup | invariant failure | violation class | cleanup defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per cleanup invariant class per attempt | invariant violation; presence fails or makes trace inconclusive | immediate stop; collect additional cleanup evidence before any further attempt |
| 1700 | PROHIBITED_COUNTERS_INITIALIZED | Info | Counters | `ChatpadEvtDeviceAdd` | before owner init | all counters | counters start zero | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add attempt | required target/request-absence evidence | no rollback; baseline for final all-zero proof |
| 1701 | TARGET_DISCOVERY_COUNTER_NONZERO | Error | Counters | future target boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1702 | TARGET_OPEN_COUNTER_NONZERO | Error | Counters | future target boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1703 | TARGET_ASSIGNMENT_COUNTER_NONZERO | Error | Counters | future target boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1704 | REQUEST_FORMAT_COUNTER_NONZERO | Error | Counters | future request boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1705 | REQUEST_REUSE_COUNTER_NONZERO | Error | Counters | future request boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1706 | REQUEST_SEND_COUNTER_NONZERO | Error | Counters | future request boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1707 | COMPLETION_COUNTER_NONZERO | Error | Counters | future completion boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1708 | CANCELLATION_COUNTER_NONZERO | Error | Counters | future cancellation boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1709 | PROTOCOL_TRAFFIC_COUNTER_NONZERO | Error | Counters | future protocol boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1710 | KEYBOARD_INJECTION_COUNTER_NONZERO | Error | Counters | future presentation boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1711 | D0_OWNER_OBSERVER_COUNTER_NONZERO | Error | Counters | future D0 owner boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1712 | REMOVAL_RUNDOWN_COUNTER_NONZERO | Error | Counters | future removal boundary | increment site | counter value | prohibited operation | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per counter's first transition from zero | required target/request-absence evidence; any nonzero event is immediate failure | immediate stop and no further runtime attempt; rollback per active state |
| 1713 | PROHIBITED_COUNTERS_FINAL_SNAPSHOT | Info/Error | Counters | terminal and cleanup paths | final/callback | all counters | zero proof or stop | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once in final terminal snapshot and once per cleanup callback | required target/request-absence evidence; final all-zero snapshot is mandatory for success | all-zero permits success interpretation; nonzero is immediate failure |
| 1800 | ATTEMPT_ID_WRAPAROUND | Error | Invariant | attempt allocator | wrap detected | previous value | instrumentation invariant | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per wraparound detection | invariant violation; presence fails first-load interpretation | immediate stop; no further runtime attempt until design is corrected |
| 1801 | EVENT_SEQUENCE_GAP_DETECTED | Error | Invariant | final validation | sequence mismatch | expected/actual class | trace/control defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per attempt final validation | invariant violation; presence makes trace inconclusive or failed | mark trace inconclusive and collect additional evidence |
| 1802 | UNEXPECTED_STATUS_MAPPING | Error | Invariant | mapping helper | default or contradiction | result/status class | taxonomy defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per mapping defect per attempt | invariant violation; presence fails first-load interpretation | return `STATUS_INVALID_DEVICE_STATE`; stop further runtime attempts |
| 1803 | OBJECT_SNAPSHOT_INCONSISTENT | Error | Invariant | snapshot helper | impossible flags | object flags | state defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per snapshot inconsistency per attempt | invariant violation; presence fails or makes trace inconclusive | immediate stop; require cleanup/rollback evidence before any future attempt |
| 1804 | COUNTER_OVERFLOW | Error | Invariant | counter increment helper | overflow | counter ID | counter defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per counter overflow | invariant violation; presence fails first-load interpretation | immediate stop; saturated counter remains nonzero failure evidence |
| 1805 | TRACE_SCHEMA_VERSION_MISMATCH | Error | Invariant | init/final | schema mismatch check | schema version | collection/parser defect | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | at most once per attempt or trace parse | invariant violation; presence makes trace inconclusive | mark trace inconclusive; collect schema/version evidence |
| 1900 | DEVICE_ADD_SUCCESS | Info | Terminal | `ChatpadEvtDeviceAdd` | final success return | final status, summary | load criterion success | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per successful device-add attempt | terminal evidence; mandatory in conclusive success | final success evidence only |
| 1901 | DEVICE_ADD_FAILURE | Error | Terminal | `ChatpadEvtDeviceAdd` | every failure return | final status, reason | terminal failure | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per failed device-add attempt | terminal evidence; required failure evidence | return recorded failure status; operator follows failure class and rollback plan |
| 1902 | DEVICE_ADD_FINAL_SUMMARY | Info/Error | Terminal | terminal/cleanup paths | before every return and cleanup | report, snapshot, counters | audit summary | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add return and once per cleanup callback | terminal evidence; mandatory for conclusive success or failure | no separate control-flow change; missing summary makes trace inconclusive |
| 1903 | DEVICE_ADD_RETURNED_STATUS | Info/Error | Terminal | `ChatpadEvtDeviceAdd` | immediately before return | returned `NTSTATUS` | exact return proof | PASSIVE_LEVEL expected; implementation must verify with source and KMDF callback contract | once per device-add attempt | terminal evidence; mandatory in conclusive success or failure | return the recorded status; final operator action follows terminal result |

Design-time catalogue validation summary:

- event count: 73;
- unique IDs: 73;
- unique symbolic names: 73;
- duplicate IDs: 0;
- duplicate names: 0;
- out-of-range IDs: 0;
- events missing expected IRQL: 0;
- events missing maximum frequency: 0;
- events missing first-load criterion: 0;
- events missing failure/rollback action: 0.

This is a documentation consistency statement. It is not a claim that a guard
has run.
...
## 29. Explicit design remediation verdict

**DESIGN REMEDIATION COMPLETE; INDEPENDENT AUDIT PENDING.**

The original design checkpoint at
`b26514f59e9d07fbee09e8bab5ea296d18c0dd85` failed independent audit for two
documentation-only reasons: incomplete per-event metadata and premature
continuity-document advancement to implementation. This remediated design has
not yet passed independent audit. Instrumentation implementation is not
...
Only after that audit passes may an offline implementation prompt be prepared.
Signing, packaging, device discovery, staging, installation, loading, runtime,
