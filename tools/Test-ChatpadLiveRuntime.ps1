...
Check ($runtime.Contains('UrbControlTransfer.PipeHandle = NULL') -and $runtime.Contains('transferFlags = USBD_DEFAULT_PIPE_TRANSFER') -and $runtime.Contains('transferFlags |= USBD_TRANSFER_DIRECTION_IN | USBD_SHORT_TRANSFER_OK') -and $runtime.Contains('URB_FUNCTION_CONTROL_TRANSFER') -and $runtime.Contains('UrbControlTransfer.SetupPacket[0] = step->Request.RawBmRequestType') -and $runtime.Contains('UrbControlTransfer.SetupPacket[7]') -and $runtime.Contains('L"TransportArchitecture", 8u') -and -not $runtime.Contains('ChatpadLiveCaptureControlPipe') -and -not $runtime.Contains('UsbBuildVendorRequest')) 'activation targets endpoint zero with the required default-pipe transfer flag in both directions'
...
Check ($runtime.Contains('ChatpadLiveSendInterfaceCommand') -and $runtime.Contains('setupPacket[0] = 0x41u') -and $runtime.Contains('setupPacket[1] = 0x00u') -and $runtime.Contains('setupPacket[4] = 0x02u') -and $runtime.Contains('setupPacket[6] = 0x00u')) 'Chatpad commands use the exact zero-length interface control request'
...
Check ($policyTests.Contains('CHATPAD_OPTIONAL_STAGE_ACTIVATION_STEP_1') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_ACTIVATION_STEP_6') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_KEEPALIVE') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_BACKLIGHT_COMMAND') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_KEYBOARD_SUBMISSION')) 'forced-failure matrix covers activation, interface commands, and keyboard submission'
