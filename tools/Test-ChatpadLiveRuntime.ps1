...
Check ($inf.Contains('USB\VID_045E&PID_028E') -and $inf.Contains('DriverVer   = 07/12/2026,1.0.11.0')) 'INF targets exact device at version 1.0.11.0'
...
Check ($runtime.Contains('UrbControlTransfer.PipeHandle = NULL') -and $runtime.Contains('transferFlags = USBD_DEFAULT_PIPE_TRANSFER') -and $runtime.Contains('transferFlags |= USBD_TRANSFER_DIRECTION_IN | USBD_SHORT_TRANSFER_OK') -and $runtime.Contains('URB_FUNCTION_CONTROL_TRANSFER') -and $runtime.Contains('UrbControlTransfer.SetupPacket[0] = step->Request.RawBmRequestType') -and $runtime.Contains('UrbControlTransfer.SetupPacket[7]') -and $runtime.Contains('L"TransportArchitecture", 6u') -and -not $runtime.Contains('ChatpadLiveCaptureControlPipe') -and -not $runtime.Contains('UsbBuildVendorRequest')) 'activation targets endpoint zero with the required default-pipe transfer flag in both directions'
...
Check ($runtime.Contains('vhfConfig.VersionNumber = 0x010B')) 'VHF child version matches package version 1.0.11.0'
