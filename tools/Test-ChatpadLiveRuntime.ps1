...
Check ($inf.Contains('USB\VID_045E&PID_028E') -and $inf.Contains('DriverVer   = 07/12/2026,1.0.12.0')) 'INF targets exact device at version 1.0.12.0'
...
Check ($header.Contains('CHATPAD_CONTROLLER_INPUT_INTERFACE_INDEX ((UCHAR)0u)') -and $header.Contains('CHATPAD_CONTROLLER_INPUT_PIPE_INDEX ((UCHAR)0u)') -and $header.Contains('CHATPAD_INPUT_INTERFACE_INDEX ((UCHAR)2u)') -and $header.Contains('CHATPAD_INPUT_PIPE_INDEX ((UCHAR)0u)')) 'controller readiness and Chatpad input use their exact configured pipes'
...
Check ($runtime.Contains('UrbControlTransfer.PipeHandle = NULL') -and $runtime.Contains('transferFlags = USBD_DEFAULT_PIPE_TRANSFER') -and $runtime.Contains('transferFlags |= USBD_TRANSFER_DIRECTION_IN | USBD_SHORT_TRANSFER_OK') -and $runtime.Contains('URB_FUNCTION_CONTROL_TRANSFER') -and $runtime.Contains('UrbControlTransfer.SetupPacket[0] = step->Request.RawBmRequestType') -and $runtime.Contains('UrbControlTransfer.SetupPacket[7]') -and $runtime.Contains('L"TransportArchitecture", 7u') -and -not $runtime.Contains('ChatpadLiveCaptureControlPipe') -and -not $runtime.Contains('UsbBuildVendorRequest')) 'activation targets endpoint zero with the required default-pipe transfer flag in both directions'
Check ($runtime.Contains('ChatpadLiveControllerInputCompletion') -and $runtime.Contains('runtime->ControllerInputPipeHandle') -and $runtime.Contains('L"ControllerInputReady"') -and $runtime.Contains('InterlockedCompareExchange(&runtime->ControllerInputReady, 0, 0) == 0') -and $runtime.Contains('InterlockedCompareExchange(&runtime->ControllerInputReady, 1, 0) == 0') -and $runtime.Contains('ChatpadLiveQueueActivationIfReady(runtime)')) 'activation waits for a successful parent controller-input completion'
...
Check ($header.Contains('CHATPAD_KEEPALIVE_INTERVAL_MS ((ULONG)1000u)') -and $header.Contains('CHATPAD_KEEPALIVE_VALUE_A ((USHORT)0x001Fu)') -and $header.Contains('CHATPAD_KEEPALIVE_VALUE_B ((USHORT)0x001Eu)')) 'Chatpad keep-alive interval and exact alternating values are fixed'
Check ($runtime.Contains('ChatpadLiveSendKeepAlive') -and $runtime.Contains('setupPacket[0] = 0x41u') -and $runtime.Contains('setupPacket[1] = 0x00u') -and $runtime.Contains('setupPacket[4] = 0x02u') -and $runtime.Contains('setupPacket[6] = 0x00u')) 'Chatpad keep-alive uses the exact zero-length interface control request'
Check ($runtime.Contains('runtime->NextKeepAliveValue == CHATPAD_KEEPALIVE_VALUE_A') -and $runtime.Contains('CHATPAD_KEEPALIVE_VALUE_B : CHATPAD_KEEPALIVE_VALUE_A') -and $runtime.Contains('KeQueryInterruptTime()')) 'input lifetime alternates keep-alives on a monotonic one-second schedule'
...
Check ($runtime.Contains('vhfConfig.VersionNumber = 0x010C')) 'VHF child version matches package version 1.0.12.0'
...
Check ($policyTests.Contains('CHATPAD_OPTIONAL_STAGE_ACTIVATION_STEP_1') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_ACTIVATION_STEP_6') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_KEEPALIVE') -and $policyTests.Contains('CHATPAD_OPTIONAL_STAGE_KEYBOARD_SUBMISSION')) 'forced-failure matrix covers activation, keep-alive, and keyboard submission'
