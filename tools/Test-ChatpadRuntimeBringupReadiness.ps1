...
Invoke-Fixture powershell-inventory-reconciliation powershell-inventory PowerShellAstInventory PASS POWERSHELL_INVENTORY_VALID @() {
    ...
    $defects=@()
    if($ps1.Count -ne 46){$defects+="tracked-ps1-count:$($ps1.Count)"}
    if($psm1.Count -ne 8){$defects+="tracked-psm1-count:$($psm1.Count)"}
    if($tracked.Count -ne 54){$defects+="tracked-total-count:$($tracked.Count)"}
...
$result=[pscustomobject][ordered]@{
    ...
    compiled_artifact_metadata_review_status='STATIC_METADATA_PARSER_IMPLEMENTATION_DESIGN_PENDING_AUDIT'
    compiled_artifact_metadata_review_authorized=$false
    compiled_artifact_metadata_review_performed=$false
    compiled_artifact_bytes_opened=$false
    compiled_artifact_reflection_performed=$false
    compiled_artifact_execution_performed=$false
    native_interop_compile_only_validation=(Test-ChatpadNativeInteropCompileOnlyValidationEvidence)
