...
"@
    ...
    $compileEvidenceMutation = {
        ...
        $source = Join-Path (git rev-parse --show-toplevel).Trim() 'docs/evidence/native-interop-compile-only-validation.json'
        $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('chatpad-native-compile-evidence-' + [guid]::NewGuid().ToString('N'))
    ...
    $compileOnlyCases = @(
        ...
        @{ id='G144'; title='compile-only remaining blocker is exact'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; [pscustomobject]@{checks=@(($v.evidence.readiness_transition.remaining_blocker-eq$nativeOperationBlocker),($v.evidence.readiness_transition.resulting_readiness_gate-eq$nativeOperationBlocker),($v.evidence.prohibited_actions.nativeInvocationOccurred-eq$false));details=$v.evidence.readiness_transition} } },
