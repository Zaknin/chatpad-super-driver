...
function Invoke-ChatpadExactInstanceOfflineSuite {
    ...
    $scaffoldAuditGate = 'BLOCKED_PENDING_INDEPENDENT_NATIVE_INTEROP_COMPILE_ONLY_REAUDIT'
...
"@
    ...
    $results.Add((Invoke-ChatpadExactCase G75 'remediated scaffold remains blocked pending independent re-audit' {
        $adapter=New-ChatpadProductionNativeAdapter
    ...
    $compileOnlyCases = @(
        ...
        @{ id='G142'; title='compile-only gate transitions only on successful compile'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; [pscustomobject]@{checks=@(($v.evidence.build_result.result-eq'PASS'),($v.evidence.readiness_transition.previous_gate-eq$historicalCompileOnlyGate),($v.evidence.readiness_transition.resulting_readiness_gate-eq$scaffoldAuditGate),($v.evidence.readiness_transition.transition_allowed-eq$true));details=$v.evidence.readiness_transition} } },
        ...
        @{ id='G144'; title='compile-only remaining blocker is exact'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; [pscustomobject]@{checks=@(($v.evidence.readiness_transition.remaining_blocker-eq$nativeOperationBlocker),($v.evidence.readiness_transition.resulting_readiness_gate-eq$scaffoldAuditGate),($v.evidence.prohibited_actions.nativeInvocationOccurred-eq$false));details=$v.evidence.readiness_transition} } },
        ...
        @{ id='G154'; title='all compile inputs use canonical LF text identity'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; $i=@($v.evidence.input_files); [pscustomobject]@{checks=@(($i.Count-eq5),(@($i|Where-Object hash_policy -ne'canonical_lf_text').Count-eq0),(@($i|Where-Object content_classification -ne'text').Count-eq0),(@($i|Where-Object line_ending_policy -ne'utf8_no_bom_lf').Count-eq0),(@($i|Where-Object canonical_sha256 -notmatch'^[A-F0-9]{64}$').Count-eq0));details=$i} } },
        ...
        @{ id='G157'; title='missing input hash policy is rejected'; body={ $v=&$compileEvidenceMutation{param($x)[void]$x.input_files[0].PSObject.Properties.Remove('hash_policy')}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-hash-policy-invalid').Count-eq1));details=$v.defects} } },
        @{ id='G158'; title='unknown input hash policy is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.input_files[0].hash_policy='unknown_policy'}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-hash-policy-invalid').Count-eq1));details=$v.defects} } },
        @{ id='G159'; title='canonical input hash mismatch is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.input_files[0].canonical_sha256=('0'*64);$x.input_files[0].sha256=('0'*64)}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-canonical-identity-mismatch').Count-eq1));details=$v.defects} } },
        @{ id='G160'; title='canonical input size mismatch is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.input_files[0].canonical_byte_size=[long]$x.input_files[0].canonical_byte_size+1;$x.input_files[0].byte_size=[long]$x.input_files[0].byte_size+1}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-canonical-identity-mismatch').Count-eq1));details=$v.defects} } },
        @{ id='G161'; title='raw policy on tracked text input is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.input_files[0].hash_policy='raw_file_bytes'}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-hash-policy-invalid').Count-eq1));details=$v.defects} } },
        @{ id='G162'; title='compile outputs retain raw file byte identity'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; $o=@($v.evidence.build_result.produced_files); [pscustomobject]@{checks=@(($o.Count-ge1),(@($o|Where-Object hash_policy -ne'raw_file_bytes').Count-eq0),(@($o|Where-Object{$_.sha256-cne$_.raw_file_sha256-or$_.byte_size-ne$_.raw_file_byte_size}).Count-eq0));details=[pscustomobject]@{count=$o.Count}} } },
        @{ id='G163'; title='canonical text policy on compile output is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.build_result.produced_files[0].hash_policy='canonical_lf_text'}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'produced-file-policy-invalid').Count-eq1));details=$v.defects} } },
        @{ id='G164'; title='compile output loading claim is rejected'; body={ $v=&$compileEvidenceMutation{param($x)$x.build_result.produced_files[0].assembly_loaded=$true}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'produced-file-prohibited-action-invalid').Count-eq1));details=$v.defects} } },
        ...
        @{ id='G168'; title='remediated compile-only phase remains pending independent reaudit'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; [pscustomobject]@{checks=@(($v.evidence.readiness_transition.resulting_readiness_gate-eq'BLOCKED_PENDING_INDEPENDENT_NATIVE_INTEROP_COMPILE_ONLY_REAUDIT'),($v.evidence.readiness_transition.remaining_blocker-eq$nativeOperationBlocker));details=$v.evidence.readiness_transition} } },
        @{ id='G169'; title='canonical aliases cannot silently claim raw identity'; body={ $v=&$compileEvidenceMutation{param($x)$x.input_files[0].sha256=$x.input_files[0].raw_working_tree_sha256}; [pscustomobject]@{checks=@(($v.result-eq'FAIL'),(@($v.defects|Where-Object id -eq'input-file-identity-alias-mismatch').Count-eq1));details=$v.defects} } },
        @{ id='G170'; title='compile input identity binds represented commit'; body={ $v=Test-ChatpadNativeInteropCompileOnlyValidationEvidence; $i=@($v.evidence.input_files); [pscustomobject]@{checks=@(($i.Count-eq5),(@($i|Where-Object commit_represented -ne$v.evidence.repository.repository_commit_at_validation).Count-eq0));details=[pscustomobject]@{commit=$v.evidence.repository.repository_commit_at_validation}} } }
    )
