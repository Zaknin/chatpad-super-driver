...
function Test-UnsignedPackageObjects([object]$Evidence,[object]$UnsignedPlan) {
    ...
    $status='CANONICAL_WINDOWS_11_DRIVER_PACKAGE_BUILT_AND_CATALOGED_UNSIGNED_FOR_LOCAL_DEVELOPMENT_NOT_STAGED_NOT_INSTALLED_NOT_LOADED_NOT_INDEPENDENTLY_AUDITED'
    ...
    $next='TASK 8I-P1B-LD-R2 '+[char]0x2014+' Independent read-only audit of the unsigned canonical package and local-development deployment contract'
    ...
    $pTop=@('schema','status','readiness','source_commit','driver_project','canonical_inf','unsigned_sys','unsigned_catalog','package_file_inventory','configuration','toolchain','reproducibility','architecture','driver_framework','driver_type','service_name','filter_name','filter_attachment','target','driver_ver','catalog_filename','unsigned_signature_states','deployment_policy','candidate_selection','current_installability','future_local_deployment_fields','blocker','next_task')
    ...
    Check (Test-ExactPropertySet $planDeployment @('deployment_scope','current_required_route','package_signed','package_staged','package_installed','driver_loaded','local_installation_method','local_signature_enforcement_state','local_candidate_identity','local_published_inf_name','ordinary_production_signature_enforcement_loadability_claimed','diagnostic_symbol_input','optional_future_production_distribution')) 'plan-deployment-properties'
    Check ($planDeployment.deployment_scope-ceq'LOCAL_DEVELOPMENT_ONLY'-and$planDeployment.current_required_route-ceq'CONTROLLED_LOCAL_DEVELOPMENT_INSTALLATION'-and-not[bool]$planDeployment.package_signed-and-not[bool]$planDeployment.package_staged-and-not[bool]$planDeployment.package_installed-and-not[bool]$planDeployment.driver_loaded-and$planDeployment.local_installation_method-ceq'NOT_YET_OBSERVED'-and$planDeployment.local_signature_enforcement_state-ceq'NOT_YET_OBSERVED'-and$planDeployment.local_candidate_identity-ceq'NOT_YET_OBSERVED'-and$planDeployment.local_published_inf_name-ceq'NOT_YET_OBSERVED'-and-not[bool]$planDeployment.ordinary_production_signature_enforcement_loadability_claimed-and$UnsignedPlan.current_installability-ceq'NOT_CURRENTLY_INSTALLABLE_LOCAL_METHOD_AND_ENFORCEMENT_STATE_NOT_OBSERVED') 'plan-local-state'
