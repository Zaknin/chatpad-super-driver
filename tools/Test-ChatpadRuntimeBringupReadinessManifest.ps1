...
function Test-ChatpadTask8GManifestEvidence {
    ...
    if($continuityDocFiles.Count-eq$continuityDocPaths.Count){
        ...
        foreach($requiredPattern in @('TASK 8G-2 passed','execution-coordinator source lane is closed','79ec174dabd7e73f2d01021168921021bc118702','BLOCKED_NATIVE_ADAPTER_TASK_8F_NATIVE_DECLARATION_REMEDIATION_NOT_INDEPENDENTLY_AUDITED','TASK 8F-R2[\s\S]{1,160}Independent read-only audit of the corrected SetupAPI/Newdev declaration ABI and propagated identities')){if($continuityText -notmatch $requiredPattern){$defects.Add("continuity.task8g: missing-required:$requiredPattern")}}
    }
    $evidencePath=[IO.Path]::GetFullPath((Join-Path $RepositoryRoot ([string](Get-ChatpadTask8EValue $section 'evidence_path'))))
...
if($manifest.schema_version-ne'chatpad-runtime-bringup-readiness-manifest-v4'-or$manifest.real_artifact_static_metadata_review_completed-ne$true-or$manifest.framework_status-ne'PASS'-or$manifest.live_installation_readiness-ne'BLOCKED'-or$manifest.current_gate-ne'BLOCKED_NATIVE_ADAPTER_TASK_8F_NATIVE_DECLARATION_REMEDIATION_NOT_INDEPENDENTLY_AUDITED'-or$manifest.capability_blocker-ne'BLOCKED_NATIVE_ADAPTER_TASK_8F_NATIVE_DECLARATION_REMEDIATION_NOT_INDEPENDENTLY_AUDITED'-or$manifest.live_adapter_status-ne'SCAFFOLD_NON_EXECUTING'-or$manifest.live_binding_authorized-ne$false-or$manifest.manifest_generation_mode-notin@('NO_ARTIFACT_OPEN_DESIGN_GATE_AUDIT','STANDARD_READINESS_RESULT')){$defects.top_level++}
