...
function Test-ChatpadTask8GManifestEvidence {
    ...
    if($continuityDocFiles.Count-eq$continuityDocPaths.Count){
        ...
        foreach($requiredPattern in @('TASK 8G-2 passed','execution-coordinator source lane\s+is\s+closed','79ec174dabd7e73f2d01021168921021bc118702','BLOCKED_NATIVE_ADAPTER_CANONICAL_DRIVER_PACKAGE_NOT_PRODUCTION_SIGNED_AND_INDEPENDENTLY_AUDITED','TASK 8I-P1B-2B[\s\S]{1,180}Obtain and validate the externally production-signed canonical Windows 11 driver package')){if($continuityText -notmatch$requiredPattern){$defects.Add("continuity.task8g: missing-required:$requiredPattern")}}
    }
    $evidencePath=[IO.Path]::GetFullPath((Join-Path $RepositoryRoot ([string](Get-ChatpadTask8EValue $section 'evidence_path'))))
...
function Test-ChatpadUnsignedPackageManifestEvidence {
    ...
    if($null-eq$section-or$section-is[array]-or$section-is[string]){return[pscustomobject]@{result='FAIL';defect_count=1;defects=@('section: missing-or-wrong-type')}}
    $expected=[ordered]@{
        evidence_path='docs/evidence/canonical-live-apply-package-build-task-8i-p1b-2a.json';evidence_byte_size=16861;evidence_sha256='8CF6C5797A5E893610B3AD77F60CC69951C674C89303B4BCB770373194AD19C4';contract_path='tools/ExactInstance/contracts/chatpad-live-apply-unsigned-package-plan.json';contract_byte_size=4814;contract_sha256='16841847061B579DE79B684238E487BA6780265D990278D75259DF78B4FB81E9';focused_test_path='tools/Test-ChatpadLiveApplyPackageSourceContract.ps1';focused_test_byte_size=23578;focused_test_sha256='AB01993EF2C52F0F21798B6BD2DAAAE3563B828ACBEAC22D7CC2B6FCFD06756D';schema='chatpad-canonical-live-apply-package-build-evidence-v1';status='CANONICAL_WINDOWS_11_DRIVER_PACKAGE_BUILT_AND_CATALOGED_UNSIGNED_NOT_PRODUCTION_SIGNED_NOT_STAGED_NOT_INSTALLABLE_NOT_INDEPENDENTLY_AUDITED';readiness='READY_FOR_EXTERNAL_PRODUCTION_SIGNING_PREPARATION_ONLY';source_commit='4510e1f2f8a4b4e31ac40672261cf4ad94ec27c5';driver_project_path='src/driver/ChatpadFilter/ChatpadFilter.vcxproj';driver_project_sha256='5641815C63FBB0C2A7442E555A1B70F3BE4C22014DF2BA29A29E987A2D903112';inf_byte_size=1581;inf_sha256='0200FCF5E1B26F4A594ECEF936DAAEC4B8F2BC5743A9DDAE212AD8F99D05E2FC';sys_byte_size=40960;sys_sha256='E4E7BCA837F6B0C662A24CFDDC260D781FDA85E0B37CAE470D65A9716DB174BB';catalog_byte_size=1202;catalog_sha256='6F0ABF84AE68010008A0360D4DDF716E644DD8D63E669F3AA96F27047EF613FD';accepted_build_count=2;sys_reproducible=$true;architecture='AMD64';sys_signature_state='unsigned';catalog_signature_state='unsigned';production_signing_route='WHCP_HLK_PRODUCTION_SIGNING';current_installability='NOT_INSTALLABLE_UNSIGNED_PACKAGE';production_signed=$false;staged=$false;installable=$false;test_certificate_present=$false;blocker='BLOCKED_NATIVE_ADAPTER_CANONICAL_DRIVER_PACKAGE_NOT_PRODUCTION_SIGNED_AND_INDEPENDENTLY_AUDITED';next_task='TASK 8I-P1B-2B — Obtain and validate the externally production-signed canonical Windows 11 driver package';file_signing_count=0;staging_count=0;installation_count=0;native_invocation_count=0;device_query_count=0;binding_count=0;authorization_count=0;windows_mutation_count=0
    }
    ...
    foreach($test in @(
        @{n='evidence.schema';a=$evidence.schema;x=$expected.schema},@{n='evidence.status';a=$evidence.status;x=$expected.status},@{n='evidence.readiness';a=$evidence.readiness;x=$expected.readiness},@{n='evidence.source_commit';a=$evidence.source_commit;x=$expected.source_commit},@{n='evidence.project';a=$evidence.driver_project.path;x=$expected.driver_project_path},@{n='evidence.inf';a=$evidence.canonical_inf.sha256;x=$expected.inf_sha256},@{n='evidence.sys';a=$evidence.binary_validation.sha256;x=$expected.sys_sha256},@{n='evidence.cat';a=$evidence.catalog_generation.sha256;x=$expected.catalog_sha256},@{n='evidence.route';a=$evidence.external_signing_handoff.selected_route;x=$expected.production_signing_route},@{n='evidence.blocker';a=$evidence.blocker;x=$expected.blocker},@{n='evidence.next';a=$evidence.next_task;x=$expected.next_task},@{n='plan.status';a=$plan.status;x=$expected.status},@{n='plan.sys';a=$plan.unsigned_sys.sha256;x=$expected.sys_sha256},@{n='plan.cat';a=$plan.unsigned_catalog.sha256;x=$expected.catalog_sha256},@{n='plan.route';a=$plan.production_signing.route;x=$expected.production_signing_route},@{n='plan.installability';a=$plan.current_installability;x=$expected.current_installability},@{n='plan.blocker';a=$plan.blocker;x=$expected.blocker},@{n='plan.next';a=$plan.next_task;x=$expected.next_task}
    )){if($test.a-isnot[string]-or-not$test.a.Equals([string]$test.x,[StringComparison]::Ordinal)){$defects.Add("$($test.n): mismatch")}}
    if(@($evidence.package.inventory).Count-ne3-or@($evidence.catalog_generation.members).Count-ne2-or@($evidence.external_signing_handoff.driver_folder_inputs).Count-ne3-or@($plan.package_file_inventory).Count-ne3-or@($plan.production_signing.external_handoff_inventory).Count-ne4){$defects.Add('inventory: mismatch')}
    foreach($value in @($evidence.signature_validation.test_certificate_present,$evidence.signature_validation.unexpected_signer_present,$evidence.external_signing_handoff.portal_access_performed,$evidence.external_signing_handoff.submission_performed,$plan.production_signing.portal_submission_created,$plan.production_signing.test_signed_substitution_permitted)){if($value-isnot[bool]-or$value){$defects.Add('prohibited-state: nonzero-or-nonboolean')}}
    [pscustomobject]@{result=if($defects.Count){'FAIL'}else{'PASS'};defect_count=$defects.Count;defects=@($defects)}
}
...
if($manifest.schema_version-ne'chatpad-runtime-bringup-readiness-manifest-v4'-or$manifest.real_artifact_static_metadata_review_completed-ne$true-or$manifest.framework_status-ne'PASS'-or$manifest.live_installation_readiness-ne'BLOCKED'-or$manifest.current_gate-ne'BLOCKED_NATIVE_ADAPTER_CANONICAL_DRIVER_PACKAGE_NOT_PRODUCTION_SIGNED_AND_INDEPENDENTLY_AUDITED'-or$manifest.capability_blocker-ne'BLOCKED_NATIVE_ADAPTER_CANONICAL_DRIVER_PACKAGE_NOT_PRODUCTION_SIGNED_AND_INDEPENDENTLY_AUDITED'-or$manifest.live_adapter_status-ne'SCAFFOLD_NON_EXECUTING'-or$manifest.live_binding_authorized-ne$false-or$manifest.manifest_generation_mode-notin@('NO_ARTIFACT_OPEN_DESIGN_GATE_AUDIT','STANDARD_READINESS_RESULT')){$defects.top_level++}
