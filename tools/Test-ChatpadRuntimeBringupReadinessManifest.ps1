...
function Test-ChatpadTask8FR1Scalar {
    param([Parameter(Mandatory)][object]$Object,[Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][object]$Expected,[Parameter(Mandatory)][string]$Location,[Parameter(Mandatory)][AllowEmptyCollection()][Collections.Generic.List[string]]$Defects)
    $property=$Object.PSObject.Properties[$Name]
    if($null-eq$property){$Defects.Add("$Location.${Name}: missing");return}
    $actual=$property.Value
    if($Expected-is[bool]){if($actual-isnot[bool]-or$actual-ne$Expected){$Defects.Add("$Location.${Name}: boolean mismatch")};return}
    if($Expected-is[int]-or$Expected-is[long]){if(($actual-isnot[int]-and$actual-isnot[long])-or[long]$actual-ne[long]$Expected){$Defects.Add("$Location.${Name}: integer mismatch")};return}
    if($actual-isnot[string]-or-not[string]::Equals([string]$Expected,[string]$actual,[StringComparison]::Ordinal)){$Defects.Add("$Location.${Name}: string mismatch")}
}

function Get-ChatpadTask8FR1FileIdentity {
    param([Parameter(Mandatory)][string]$Path)
    $raw=[IO.File]::ReadAllBytes($Path)
    $rawHash=[BitConverter]::ToString(([Security.Cryptography.SHA256]::Create()).ComputeHash($raw)).Replace('-','')
    $text=[Text.Encoding]::UTF8.GetString($raw).Replace("`r`n","`n").Replace("`r","`n")
    $canonical=[Text.UTF8Encoding]::new($false).GetBytes($text)
    $canonicalHash=[BitConverter]::ToString(([Security.Cryptography.SHA256]::Create()).ComputeHash($canonical)).Replace('-','')
    [pscustomobject]@{raw_byte_size=[long]$raw.Length;raw_sha256=$rawHash;canonical_lf_byte_size=[long]$canonical.Length;canonical_lf_sha256=$canonicalHash}
}

function Test-ChatpadTask8FR1ObjectEquality {
    param([Parameter(Mandatory)][object]$Expected,[Parameter(Mandatory)][object]$Actual,[Parameter(Mandatory)][string]$Location,[Parameter(Mandatory)][AllowEmptyCollection()][Collections.Generic.List[string]]$Defects)
    if($Expected-is[System.Collections.IList]-and$Expected-isnot[string]){
        foreach($defect in (Get-ChatpadExactOrdinalArrayValueDefects -ExpectedValues $Expected -ActualValues $Actual -FieldName $Location).defects){$Defects.Add($defect)}
        return
    }
    if($Expected-is[pscustomobject]){
        if($null-eq$Actual-or$Actual-isnot[pscustomobject]){$Defects.Add("${Location}: object mismatch");return}
        foreach($property in $Expected.PSObject.Properties){
            $actualProperty=$Actual.PSObject.Properties[$property.Name]
            if($null-eq$actualProperty){$Defects.Add("$Location.$($property.Name): missing");continue}
            Test-ChatpadTask8FR1ObjectEquality -Expected $property.Value -Actual $actualProperty.Value -Location "$Location.$($property.Name)" -Defects $Defects
        }
        return
    }
    if($Expected-is[bool]){if($Actual-isnot[bool]-or$Actual-ne$Expected){$Defects.Add("${Location}: boolean mismatch")};return}
    if($Expected-is[int]-or$Expected-is[long]){if(($Actual-isnot[int]-and$Actual-isnot[long])-or[long]$Actual-ne[long]$Expected){$Defects.Add("${Location}: integer mismatch")};return}
    if($Actual-isnot[string]-or-not[string]::Equals([string]$Expected,[string]$Actual,[StringComparison]::Ordinal)){$Defects.Add("${Location}: string mismatch")}
}

function Test-ChatpadTask8FR1ManifestEvidence {
    param([Parameter(Mandatory)][string]$RepositoryRoot,[Parameter(Mandatory)][object]$Manifest)
    $defects=[Collections.Generic.List[string]]::new();$section=Get-ChatpadTask8EValue $Manifest 'native_interop_declaration_remediation'
    if($null-eq$section-or$section-is[array]){return [pscustomobject]@{result='FAIL';defect_count=1;defects=@('section')}}
    $expectedScalars=[ordered]@{
        evidence_path='docs/evidence/native-interop-declaration-remediation-task-8f-r1.json';schema='chatpad-native-interop-declaration-remediation-evidence-v1';status='TASK_8F_NATIVE_DECLARATION_ABI_CORRECTED_IDENTITY_PROPAGATED_NOT_INDEPENDENTLY_AUDITED_NO_NATIVE_EXECUTION_NO_DEVICE_QUERY_NO_BINDING_NO_MUTATION';readiness='READY_FOR_INDEPENDENT_TASK_8F_DECLARATION_REMEDIATION_AUDIT_ONLY';created_from_task='TASK 8F-R1B';starting_branch='feature/task-8f-native-declaration-remediation';starting_commit='bd56cbaed47a9a5f9ba23ea8fab075b5a6c32c57';r1a_remediation_commit='bd56cbaed47a9a5f9ba23ea8fab075b5a6c32c57';r1a_parent_commit='6186b37c6b1cacc3fb91c38011bc45525bcc1a54';implementation_commit_source='exact commit created after validation with subject fix: propagate corrected native declaration identity';declaration_path='tools/ExactInstance/NativeInterop/Chatpad.NativeInterop.SetupApiNewdev.Declarations.cs';corrected_method_signature='bool SetupDiGetDriverInstallParamsW(IntPtr DeviceInfoSet, ref SP_DEVINFO_DATA DeviceInfoData, ref SP_DRVINFO_DATA_W DriverInfoData, ref SP_DRVINSTALL_PARAMS DriverInstallParams)';blocker='BLOCKED_NATIVE_ADAPTER_TASK_8F_NATIVE_DECLARATION_REMEDIATION_NOT_INDEPENDENTLY_AUDITED';next_task='TASK 8F-R2 — Independent read-only audit of the corrected SetupAPI/Newdev declaration ABI and propagated identities'
    }
    foreach($pair in $expectedScalars.GetEnumerator()){Test-ChatpadTask8FR1Scalar -Object $section -Name $pair.Key -Expected $pair.Value -Location section -Defects $defects}
    $fixedObjects=@(
        @{name='old_identity';values=[ordered]@{raw_byte_size=9997;raw_sha256='EEE30064DEF4817ABD43E9BEFF6A6726AC125E62F96CE4F1B71C6A5FBA498703';canonical_lf_byte_size=9783;canonical_lf_sha256='127EA58993862CCE865E3D73B0F1A99513932ABDF1BEA615812966EB5C14BEAA';classification='SUPERSEDED_HISTORY_NOT_CURRENT'}},
        @{name='corrected_identity';values=[ordered]@{raw_byte_size=10238;raw_sha256='E55E6E34BBB4DB40904F065292F23A76D48BE809D18E7EB76E0C3A7ECCA786F2';canonical_lf_byte_size=10034;canonical_lf_sha256='B4D24BF374B391A36B4A3513B117A2FF50F8BC3D8795248808086AC984E874D9';hash_policy='canonical_lf_text'}},
        @{name='driver_install_params_contract';values=[ordered]@{native_name='SP_DRVINSTALL_PARAMS';managed_name='SP_DRVINSTALL_PARAMS';layout='LayoutKind.Sequential';x86_byte_size=20;x64_byte_size=32;old_sp_devinstall_params_binding_absent=$true}},
        @{name='abi_test';values=[ordered]@{path='tools/Test-ChatpadNativeInteropSetupApiDeclarations.ps1';raw_byte_size=23344;raw_sha256='0C708A93980264D96B7CE755512F4EEEB93D80B3CE65F07A4F6B1F583A9C654D';powershell_7_result='PASS';powershell_7_test_count=8;powershell_7_assertion_count=116;windows_powershell_result='PASS';windows_powershell_test_count=8;windows_powershell_assertion_count=116}},
        @{name='compile_only_validation';values=[ordered]@{result='PASS';compiler_exit_code=0;warning_count=0;error_count=0;produced_file_count=18;persistent_output_cleanup_performed=$true;persistent_assembly_or_binary_present=$false;assembly_loaded=$false;managed_code_executed=$false;native_api_invoked=$false}}
    )
    foreach($item in $fixedObjects){$object=Get-ChatpadTask8EValue $section $item.name;if($null-eq$object-or$object-is[array]){$defects.Add("section.$($item.name): object missing");continue};foreach($pair in $item.values.GetEnumerator()){Test-ChatpadTask8FR1Scalar -Object $object -Name $pair.Key -Expected $pair.Value -Location "section.$($item.name)" -Defects $defects}}
    $expectedMethods=@('SetupDiCreateDeviceInfoList','SetupDiDestroyDeviceInfoList','SetupDiOpenDeviceInfoW','SetupDiGetDeviceInstanceIdW','SetupDiGetDevicePropertyW','SetupDiGetDeviceRegistryPropertyW','SetupDiBuildDriverInfoList','SetupDiDestroyDriverInfoList','SetupDiEnumDriverInfoW','SetupDiGetDriverInfoDetailW','SetupDiGetDriverInstallParamsW','SetupDiSetSelectedDriverW','DiInstallDevice')
    ...
    $expectedFields=@('uint cbSize','uint Rank','uint Flags','UIntPtr PrivateData','uint Reserved');$expectedHeaders=@('um/setupapi.h','um/newdev.h','shared/devpropdef.h','shared/devpkey.h')
    foreach($array in @(@{o=$section;n='native_api_inventory';v=$expectedMethods;l='section.native_api_inventory'},@{o=$section;n='structure_inventory';v=$expectedStructures;l='section.structure_inventory'},@{o=(Get-ChatpadTask8EValue $section 'driver_install_params_contract');n='fields';v=$expectedFields;l='section.driver_install_params_contract.fields'},@{o=(Get-ChatpadTask8EValue $section 'sdk_provenance');n='headers';v=$expectedHeaders;l='section.sdk_provenance.headers'})){foreach($defect in (Get-ChatpadExactOrdinalArrayPropertyDefects -Object $array.o -PropertyName $array.n -ExpectedValues $array.v -FieldName $array.l).defects){$defects.Add($defect)}}
    $sdk=Get-ChatpadTask8EValue $section 'sdk_provenance';if($null-eq$sdk){$defects.Add('section.sdk_provenance')}else{Test-ChatpadTask8FR1Scalar -Object $sdk -Name sdk_version -Expected '10.0.26100.0' -Location section.sdk_provenance -Defects $defects}
    $safety=Get-ChatpadTask8EValue $section 'safety_counters';foreach($name in @('production_provider_construction_count','production_provider_invocation_count','native_invocation_count','setupapi_newdev_invocation_count','device_query_count','registry_query_count','service_query_count','binding_count','windows_mutation_count','driver_action_count','live_authorization_issue_count','live_authorization_consumption_count')){Test-ChatpadTask8FR1Scalar -Object $safety -Name $name -Expected 0 -Location section.safety_counters -Defects $defects}
    foreach($pair in ([ordered]@{live_authorization_active=$false;operator_authorization_phrase_active=$false;task_8f_closed=$false;task_8i_blocked=$true}).GetEnumerator()){Test-ChatpadTask8FR1Scalar -Object $section -Name $pair.Key -Expected $pair.Value -Location section -Defects $defects}
    $evidenceRelative=[string](Get-ChatpadTask8EValue $section 'evidence_path');$evidencePath=[IO.Path]::GetFullPath((Join-Path $RepositoryRoot $evidenceRelative))
    if(-not$evidencePath.StartsWith($RepositoryRoot+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)-or-not(Test-Path -LiteralPath $evidencePath -PathType Leaf)){$defects.Add('evidence.path');return [pscustomobject]@{result='FAIL';defect_count=$defects.Count;defects=@($defects)}}
    $evidenceIdentity=Get-ChatpadTask8FR1FileIdentity $evidencePath;Test-ChatpadTask8FR1Scalar -Object $section -Name evidence_byte_size -Expected $evidenceIdentity.raw_byte_size -Location section -Defects $defects;Test-ChatpadTask8FR1Scalar -Object $section -Name evidence_sha256 -Expected $evidenceIdentity.raw_sha256 -Location section -Defects $defects
    try{$evidence=Get-Content -LiteralPath $evidencePath -Raw|ConvertFrom-Json}catch{$defects.Add('evidence.json');return [pscustomobject]@{result='FAIL';defect_count=$defects.Count;defects=@($defects)}}
    foreach($property in $evidence.PSObject.Properties){$manifestProperty=$section.PSObject.Properties[$property.Name];if($null-eq$manifestProperty){$defects.Add("manifest-evidence.$($property.Name): missing")}else{Test-ChatpadTask8FR1ObjectEquality -Expected $property.Value -Actual $manifestProperty.Value -Location "manifest-evidence.$($property.Name)" -Defects $defects}}
    $identities=Get-ChatpadTask8EValue $section 'propagated_source_identities';foreach($name in @('compile_only_contract','source_boundary','design_gate','compile_only_producer','offline_suite','compile_only_evidence')){$record=Get-ChatpadTask8EValue $identities $name;if($null-eq$record){$defects.Add("section.propagated_source_identities.$name");continue};$path=[IO.Path]::GetFullPath((Join-Path $RepositoryRoot ([string](Get-ChatpadTask8EValue $record 'path'))));if(-not(Test-Path -LiteralPath $path -PathType Leaf)){$defects.Add("source.$name.missing");continue};$actual=Get-ChatpadTask8FR1FileIdentity $path;foreach($field in @('raw_byte_size','raw_sha256','canonical_lf_byte_size','canonical_lf_sha256')){Test-ChatpadTask8FR1Scalar -Object $record -Name $field -Expected $actual.$field -Location "section.propagated_source_identities.$name" -Defects $defects}}
    $declarationPath=Join-Path $RepositoryRoot ([string](Get-ChatpadTask8EValue $section 'declaration_path'));if(Test-Path -LiteralPath $declarationPath -PathType Leaf){$actual=Get-ChatpadTask8FR1FileIdentity $declarationPath;foreach($field in @('raw_byte_size','raw_sha256','canonical_lf_byte_size','canonical_lf_sha256')){Test-ChatpadTask8FR1Scalar -Object $section.corrected_identity -Name $field -Expected $actual.$field -Location section.corrected_identity -Defects $defects}}else{$defects.Add('source.declaration.missing')}
    [pscustomobject]@{result=if($defects.Count){'FAIL'}else{'PASS'};defect_count=$defects.Count;defects=@($defects)}
}

function Invoke-ChatpadTask8FR1Regression {
    param([Parameter(Mandatory)][string]$ManifestPath)
    $root=[IO.Path]::GetFullPath((&git rev-parse --show-toplevel).Trim());$temp=Join-Path ([IO.Path]::GetTempPath()) ('chatpad-task8fr1-'+[guid]::NewGuid().ToString('N'))
    $paths=@($ManifestPath,'docs/evidence/native-interop-declaration-remediation-task-8f-r1.json','tools/ExactInstance/NativeInterop/Chatpad.NativeInterop.SetupApiNewdev.Declarations.cs','tools/Test-ChatpadNativeInteropSetupApiDeclarations.ps1','tools/ExactInstance/CompileOnlyValidation/CompileOnlyContracts.cs','tools/ExactInstance/NativeInterop/ChatpadNativeInteropSourceBoundary.psm1','tools/ExactInstance/ChatpadNativeAdapterDesignGate.psm1','tools/Invoke-ChatpadNativeInteropCompileOnlyValidation.ps1','tools/ExactInstance/ChatpadExactInstance.OfflineSuite.psm1','docs/evidence/native-interop-compile-only-validation.json')
    New-Item -ItemType Directory -Path $temp|Out-Null
    try{
        foreach($relative in $paths){$target=Join-Path $temp $relative;New-Item -ItemType Directory -Path (Split-Path -Parent $target) -Force|Out-Null;Copy-Item -LiteralPath (Join-Path $root $relative) -Destination $target -Force}
        $manifestFile=Join-Path $temp $ManifestPath;$evidenceFile=Join-Path $temp 'docs/evidence/native-interop-declaration-remediation-task-8f-r1.json';$manifestText=Get-Content $manifestFile -Raw;$evidenceText=Get-Content $evidenceFile -Raw
        $cases=[Collections.Generic.List[object]]::new()
        $cases.Add([pscustomobject]@{id='section-removed';mutate={param($m,$e)[void]$m.PSObject.Properties.Remove('native_interop_declaration_remediation')}})
        foreach($expression in @(
            @{id='old-raw-size';x='$m.native_interop_declaration_remediation.old_identity.raw_byte_size=1'},@{id='old-raw-hash';x='$m.native_interop_declaration_remediation.old_identity.raw_sha256=''tampered'''},@{id='old-canonical-size';x='$m.native_interop_declaration_remediation.old_identity.canonical_lf_byte_size=1'},@{id='old-canonical-hash';x='$m.native_interop_declaration_remediation.old_identity.canonical_lf_sha256=''tampered'''},
            @{id='new-raw-size';x='$m.native_interop_declaration_remediation.corrected_identity.raw_byte_size=1'},@{id='new-raw-hash';x='$m.native_interop_declaration_remediation.corrected_identity.raw_sha256=''tampered'''},@{id='new-canonical-size';x='$m.native_interop_declaration_remediation.corrected_identity.canonical_lf_byte_size=1'},@{id='new-canonical-hash';x='$m.native_interop_declaration_remediation.corrected_identity.canonical_lf_sha256=''tampered'''},
            @{id='corrected-signature';x='$m.native_interop_declaration_remediation.corrected_method_signature=''old SP_DEVINSTALL_PARAMS_W binding'''},@{id='structure-name';x='$m.native_interop_declaration_remediation.driver_install_params_contract.native_name=''SP_DEVINSTALL_PARAMS_W'''},@{id='field-inventory';x='$m.native_interop_declaration_remediation.driver_install_params_contract.fields=@($m.native_interop_declaration_remediation.driver_install_params_contract.fields|Select-Object -Skip 1)'},@{id='field-order';x='$v=@($m.native_interop_declaration_remediation.driver_install_params_contract.fields);$t=$v[0];$v[0]=$v[1];$v[1]=$t;$m.native_interop_declaration_remediation.driver_install_params_contract.fields=$v'},@{id='pointer-field-type';x='$m.native_interop_declaration_remediation.driver_install_params_contract.fields[3]=''uint PrivateData'''},@{id='x86-size';x='$m.native_interop_declaration_remediation.driver_install_params_contract.x86_byte_size=24'},@{id='x64-size';x='$m.native_interop_declaration_remediation.driver_install_params_contract.x64_byte_size=24'},
            @{id='sdk-version';x='$m.native_interop_declaration_remediation.sdk_provenance.sdk_version=''tampered'''},@{id='sdk-headers';x='$m.native_interop_declaration_remediation.sdk_provenance.headers=@($m.native_interop_declaration_remediation.sdk_provenance.headers|Select-Object -Skip 1)'},@{id='method-inventory';x='$m.native_interop_declaration_remediation.native_api_inventory=@($m.native_interop_declaration_remediation.native_api_inventory|Select-Object -Skip 1)'},@{id='structure-inventory';x='$m.native_interop_declaration_remediation.structure_inventory=@($m.native_interop_declaration_remediation.structure_inventory|Select-Object -Skip 1)'},
            @{id='abi-test-identity';x='$m.native_interop_declaration_remediation.abi_test.raw_sha256=''tampered'''},@{id='abi-test-count';x='$m.native_interop_declaration_remediation.abi_test.powershell_7_assertion_count=115'},@{id='compile-evidence-identity';x='$m.native_interop_declaration_remediation.propagated_source_identities.compile_only_evidence.raw_sha256=''tampered'''},@{id='schema';x='$m.native_interop_declaration_remediation.schema=''tampered'''},@{id='status';x='$m.native_interop_declaration_remediation.status=''tampered'''},@{id='readiness';x='$m.native_interop_declaration_remediation.readiness=''tampered'''},
            @{id='production-counter';x='$m.native_interop_declaration_remediation.safety_counters.production_provider_construction_count=1'},@{id='native-counter';x='$m.native_interop_declaration_remediation.safety_counters.native_invocation_count=1'},@{id='device-counter';x='$m.native_interop_declaration_remediation.safety_counters.device_query_count=1'},@{id='mutation-counter';x='$m.native_interop_declaration_remediation.safety_counters.windows_mutation_count=1'},@{id='blocker';x='$m.native_interop_declaration_remediation.blocker=''tampered'''},@{id='next-task';x='$m.native_interop_declaration_remediation.next_task=''tampered'''},@{id='manifest-evidence-disagreement';x='$e.status=''tampered'''}
        )){$cases.Add([pscustomobject]@{id=$expression.id;mutate=[scriptblock]::Create("param(`$m,`$e) $($expression.x)")})}
        $results=[Collections.Generic.List[object]]::new();$canonical=Get-Content $manifestFile -Raw|ConvertFrom-Json;$pass=Test-ChatpadTask8FR1ManifestEvidence -RepositoryRoot $temp -Manifest $canonical;$results.Add([pscustomobject]@{id='canonical';result=$pass.result;defects=@($pass.defects);passed=($pass.result-eq'PASS')})
        foreach($case in $cases){[IO.File]::WriteAllText($manifestFile,$manifestText,[Text.UTF8Encoding]::new($false));[IO.File]::WriteAllText($evidenceFile,$evidenceText,[Text.UTF8Encoding]::new($false));$m=Get-Content $manifestFile -Raw|ConvertFrom-Json;$e=Get-Content $evidenceFile -Raw|ConvertFrom-Json;& $case.mutate $m $e;[IO.File]::WriteAllText($evidenceFile,($e|ConvertTo-Json -Depth 30)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false));$result=Test-ChatpadTask8FR1ManifestEvidence -RepositoryRoot $temp -Manifest $m;$results.Add([pscustomobject]@{id=$case.id;result=$result.result;passed=($result.result-eq'FAIL')})}
        $failed=@($results|Where-Object{-not$_.passed});[pscustomobject]@{result=if($failed.Count){'FAIL'}else{'PASS'};test_count=$results.Count;assertion_count=$results.Count;failed_test_count=$failed.Count;tests=@($results);temporary_data_removed=$true}
    }finally{if(Test-Path $temp){Remove-Item -LiteralPath $temp -Recurse -Force}}
}

function Get-ChatpadTask8GExpectedScalars {
    [ordered]@{
        schema='chatpad-one-shot-native-execution-coordinator-evidence-v2';status='ATOMIC_ONE_SHOT_EXECUTION_COORDINATOR_SOURCE_PRESENT_GENUINE_RECORDING_PROVIDER_ONLY_PRODUCTION_PROVIDER_NOT_SELECTED_OR_CONSTRUCTED_NO_NATIVE_EXECUTION_NO_BINDING_NO_MUTATION_LIVE_EXECUTION_UNAUTHORIZED';readiness='READY_FOR_INDEPENDENT_SOURCE_AUDIT_ONLY';created_from_task='TASK 8G-1R5';starting_branch='feature/native-adapter-production-backend-source';starting_commit='9147f0af7e8fa8da581554427ebd51908e5d924e';implementation_branch='feature/native-adapter-execution-coordinator';atomic_implementation_commit='182ba40a0ccb39974838730f324a9c9ce41b8475';implementation_commit_parent='3baca0fb97d80ec61bae9e07fd62d12cb21e733d';implementation_commit_subject='fix: eliminate coordinator provider injection surface';remediation_parent_commit='3baca0fb97d80ec61bae9e07fd62d12cb21e733d';coordinator_path='tools/ExactInstance/ChatpadOneShotNativeExecutionCoordinator.psm1';coordinator_byte_size=8412;coordinator_sha256='B639AAB4D3FBA440E1DD4183C71F8D5FCF54B632CF3DE2EAA91E50535B7277DA';managed_helper_path='tools/ExactInstance/ChatpadOneShotAuthorizationRegistry.cs';managed_helper_byte_size=1101;managed_helper_sha256='9C85311242920C0C2DDCDAD9A7AED9A10C6F145F70DFA03B092607372485A462';focused_test_path='tools/Test-ChatpadOneShotNativeExecutionCoordinator.ps1';focused_test_byte_size=20817;focused_test_sha256='5DE28256C6E3B4FB62A1CEA1A4307FFB3A2600D4FC9580C79D3126623AFD8412';task_8e_evidence_path='docs/evidence/native-adapter-nonexecuting-implementation-task-8e-1.json';task_8e_evidence_sha256='493C80C445B9D3903D70509AA704177396172B139CCDD037195D4FA84AA9FB6F';task_8f_evidence_path='docs/evidence/native-adapter-production-backend-source-task-8f-1.json';task_8f_evidence_sha256='93B0D93E63FA43A9D5C7EE647BD5E261085CD3A4F47F5CD4FD4D1DFA01800042';capability_mechanism='REFERENCE_KEYED_OFFLINE_RECORDING_AUTHORIZATION_WITH_PRIVATE_PROVIDER_MAP';authorization_registry_mechanism='CONDITIONAL_WEAK_TABLE_WITH_INTERLOCKED_COMPARE_EXCHANGE_NO_PROVIDER_STORAGE';provider_provenance_mechanism='INTERNAL_TASK_8F_RECORDING_PROVIDER_PRIVATE_REFERENCE_MAP';provider_binding_source='INTERNAL_TASK_8F_RECORDING_PROVIDER_ONLY';atomic_consumption_primitive='System.Threading.Interlocked.CompareExchange';same_process_concurrency_mechanism='TWO_POWERSHELL_RUNSPACES_SHARING_ONE_MANAGED_CONDITIONAL_WEAK_TABLE_REGISTRY';caller_supplied_provider_accepted=$false;arbitrary_provider_registration_allowed=$false;production_provider_bindable_by_test_factory=$false;managed_registration_accepts_provider_parameter=$false;public_managed_provider_injection_surface=$false;obsolete_arbitrary_provider_registration_signature_present=$false;provider_replacement_method_present=$false;authorization_reset_method_present=$false;consumed_state_reset_method_present=$false;provider_stored_in_authorization_record=$false;provider_selected_from_caller_input=$false;provider_provenance_uses_reference_identity=$true;recording_provider_created_internally=$true;direct_arbitrary_provider_managed_boundary_tests_passed=$true;direct_production_descriptor_managed_boundary_test_passed=$true;consumption_atomic=$true;consume_before_provider_call=$true;concurrency_test_uses_actual_registry=$true;race_iteration_count=16;bounded_race_iteration_count=16;concurrent_contender_count=2;concurrent_winner_count=1;concurrent_provider_entry_count=1;concurrent_replay_rejection_count=1;replay_after_success_rejected=$true;replay_after_provider_failure_rejected=$true;replay_after_exception_rejected=$true;replay_after_cleanup_failure_rejected=$true;request_transfer_rejected=$true;provider_substitution_rejected=$true;wrapper_authorization_rejected=$true;deserialized_authorization_rejected=$true;forged_type_authorization_rejected=$true;string_authorization_rejected=$true;boolean_authorization_rejected=$true;accepted_operation='APPLY';shared_container_id='{828F4587-006F-5AD1-B169-6AF57905DFDE}';operator_confirmation_id='operator-confirmation-c445630fbb303c7c';recording_provider_plan_entry_count=13;recording_provider_plan_identity='TASK_8F_13_STEP_RECORDING_PLAN';failure_stop_behavior='SIMULATED_FAILURE_STOPS_NORMAL_STEPS';cleanup_behavior='TASK_8F_REQUIRED_CLEANUP_RECORDED';thrown_exception_behavior='TEST_LOCAL_RECORDING_PROVIDER_CALL_WRAPPER_THROWS_AFTER_GENUINE_RECORDING_ENTRY';cleanup_failure_behavior='TEST_LOCAL_RECORDING_PROVIDER_CALL_WRAPPER_THROWS_AFTER_GENUINE_CLEANUP_ENTRY';production_provider_selected=$false;production_provider_constructed=$false;production_provider_loaded=$false;production_provider_invoked=$false;execution_authorized=$false;native_execution_performed=$false;binding_performed=$false;windows_mutation_performed=$false;powershell_7_result='PASS';powershell_7_test_count=25;powershell_7_assertion_count=259;windows_powershell_result='PASS';windows_powershell_test_count=25;windows_powershell_assertion_count=259;device_query_count=0;native_invocation_count=0;setupapi_newdev_invocation_count=0;binding_count=0;windows_mutation_count=0;driver_action_count=0;rollback_count=0;restore_count=0;artifact_compile_output_access_count=0;artifact_access_count=0;compile_output_access_count=0;live_readiness='BLOCKED';remaining_blocker='BLOCKED_NATIVE_ADAPTER_EXECUTION_COORDINATOR_NOT_INDEPENDENTLY_AUDITED';ready_for_independent_audit_only=$true;ready_for_native_execution=$false
    }
}

function Get-ChatpadTask8GExpectedArrays {
    [ordered]@{
        public_exports=@('Get-ChatpadNonExecutingNativeAdapterContract','Test-ChatpadNonExecutingNativeAdapterRequest','Invoke-ChatpadNonExecutingNativeAdapter','New-ChatpadNativeAdapterFakeRecordingBackend');private_coordinator_surface=@('Get-OneShotCoordinatorConstants','Get-OneShotRequestFingerprint','Test-OneShotExecutionRequest','New-TestOnlyRecordingProviderAuthorization','Invoke-OneShotNativeExecutionCoordinator');managed_authorization_helper_surface=@('Chatpad.OneShotAuthorization.Registry.CreateRecordingAuthorization','Chatpad.OneShotAuthorization.Registry.TryConsume');managed_method_signatures=@('System.Void Chatpad.OneShotAuthorization.Registry.CreateRecordingAuthorization(System.Object key, System.String fingerprint)','System.Boolean Chatpad.OneShotAuthorization.Registry.TryConsume(System.Object key, System.String fingerprint)');managed_constructor_signatures=@();request_binding_fields=@('operation','ordered_target_chain','container_id','operator_confirmation_id','task_8e_evidence_identity','task_8f_evidence_identity','private_recording_provider_reference_map');accepted_ordered_target_chain=@('USB\VID_045E&PID_028E\1C21F10','USB\VID_045E&PID_028E&IG_00\8&2AF61D70&1&00','HID\VID_045E&PID_028E&IG_00\9&2E72F677&0&0000');recording_provider_plan=@('SetupDiCreateDeviceInfoList','SetupDiOpenDeviceInfoW','SetupDiGetDeviceInstanceIdW','SetupDiGetDevicePropertyW','SetupDiGetDeviceRegistryPropertyW','SetupDiBuildDriverInfoList','SetupDiEnumDriverInfoW','SetupDiGetDriverInfoDetailW','SetupDiGetDriverInstallParamsW','SetupDiSetSelectedDriverW','DiInstallDevice','SetupDiDestroyDriverInfoList','SetupDiDestroyDeviceInfoList');outcome_inventory=@('replay_after_success_rejected','replay_after_provider_failure_rejected','replay_after_thrown_exception_rejected','replay_after_cleanup_failure_rejected','request_transfer_rejected','provider_substitution_rejected','wrapper_authorization_rejected','deserialized_authorization_rejected','forged_type_authorization_rejected','string_authorization_rejected','boolean_authorization_rejected','direct_arbitrary_provider_registration_rejected','direct_production_descriptor_registration_rejected')
    }
}

function Test-ChatpadTask8GExpectedScalar {
    param([Parameter(Mandatory)][object]$Object,[Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][object]$Expected,[Parameter(Mandatory)][string]$Location,[Parameter(Mandatory)][AllowEmptyCollection()][Collections.Generic.List[string]]$Defects)
    $property=$Object.PSObject.Properties[$Name]
    if($null-eq$property){$Defects.Add("$Location.${Name}: missing");return}
    $actual=$property.Value
    if($Expected-is[bool]){if($actual-isnot[bool]-or$actual-ne$Expected){$Defects.Add("$Location.${Name}: boolean mismatch")};return}
    if($Expected-is[int]-or$Expected-is[long]){if($actual-is[string]-or$actual-is[array]-or$actual-is[bool]-or$actual-isnot[int]-and$actual-isnot[long]-or[long]$actual-ne[long]$Expected){$Defects.Add("$Location.${Name}: integer mismatch")};return}
    if($actual-isnot[string]-or$actual-cne$Expected){$Defects.Add("$Location.${Name}: string mismatch")}
}

function Test-ChatpadTask8GManifestEvidence {
    param([Parameter(Mandatory)][string]$RepositoryRoot,[Parameter(Mandatory)][object]$Manifest)
    $defects=[Collections.Generic.List[string]]::new();$section=Get-ChatpadTask8EValue $Manifest 'one_shot_native_execution_coordinator'
    ...
    $expected=Get-ChatpadTask8GExpectedScalars;$arrays=Get-ChatpadTask8GExpectedArrays;$manifestIdentity=[ordered]@{evidence_path='docs/evidence/one-shot-native-execution-coordinator-task-8g-1.json';evidence_byte_size=8165;evidence_sha256='BB2016A9CFD85BFCA06862E7B3D386EE611355AA2416778BFDCB49C7A6C0BD12'}
    foreach($pair in $manifestIdentity.GetEnumerator()){Test-ChatpadTask8GExpectedScalar -Object $section -Name $pair.Key -Expected $pair.Value -Location 'section' -Defects $defects}
    ...
    foreach($identity in @(@{path='tools/ExactInstance/ChatpadOneShotNativeExecutionCoordinator.psm1';size=8412;hash='B639AAB4D3FBA440E1DD4183C71F8D5FCF54B632CF3DE2EAA91E50535B7277DA';prefix='coordinator'},@{path='tools/ExactInstance/ChatpadOneShotAuthorizationRegistry.cs';size=1101;hash='9C85311242920C0C2DDCDAD9A7AED9A10C6F145F70DFA03B092607372485A462';prefix='managed_helper'},@{path='tools/Test-ChatpadOneShotNativeExecutionCoordinator.ps1';size=20817;hash='5DE28256C6E3B4FB62A1CEA1A4307FFB3A2600D4FC9580C79D3126623AFD8412';prefix='focused_test'})){
        $file=Join-Path $RepositoryRoot $identity.path;if(-not(Test-Path -LiteralPath $file -PathType Leaf)){$defects.Add("source.$($identity.prefix): missing");continue};if((Get-Item -LiteralPath $file).Length-ne$identity.size){$defects.Add("source.$($identity.prefix): byte-size")};if((Get-FileHash -LiteralPath $file -Algorithm SHA256).Hash-cne$identity.hash){$defects.Add("source.$($identity.prefix): sha256")}
    }
    $managedHelperSource=Get-Content -LiteralPath (Join-Path $RepositoryRoot 'tools/ExactInstance/ChatpadOneShotAuthorizationRegistry.cs') -Raw
    ...
    if($continuityDocFiles.Count-eq$continuityDocPaths.Count){
        ...
        foreach($requiredPattern in @('TASK 8G-2 passed','execution-coordinator source lane\s+is\s+closed','79ec174dabd7e73f2d01021168921021bc118702','BLOCKED_NATIVE_ADAPTER_SELF_TEST_SIGNED_PACKAGE_NOT_CREATED_AND_CONFIGURED_TESTSIGNING_NOT_EFFECTIVE_UNTIL_REBOOT','TASK 8I-P1B-LD-S1[\s\S]{1,220}Create and freeze the self-test-signed local-development package')){if($continuityText -notmatch$requiredPattern){$defects.Add("continuity.task8g: missing-required:$requiredPattern")}}
    }
    $evidencePath=[IO.Path]::GetFullPath((Join-Path $RepositoryRoot ([string](Get-ChatpadTask8EValue $section 'evidence_path'))))
    ...
    foreach($pair in $arrays.GetEnumerator()){
        ...
        $sectionArrayProperty=$section.PSObject.Properties[$pair.Key];$evidenceArrayProperty=$evidence.PSObject.Properties[$pair.Key]
        if($null-eq$sectionArrayProperty-or$null-eq$evidenceArrayProperty){$defects.Add("manifest-evidence.$($pair.Key): property is missing.")}
        else{foreach($defect in (Get-ChatpadExactOrdinalArrayValueDefects -ExpectedValues $sectionArrayProperty.Value -ActualValues $evidenceArrayProperty.Value -FieldName "manifest-evidence.$($pair.Key)").defects){$defects.Add($defect)}}
    }
    [pscustomobject]@{result=if($defects.Count){'FAIL'}else{'PASS'};defect_count=$defects.Count;defects=@($defects)}
}

function Invoke-ChatpadTask8GRegression {
    param([Parameter(Mandatory)][string]$ManifestPath)
    $root=[IO.Path]::GetFullPath((&git rev-parse --show-toplevel).Trim());$temp=Join-Path ([IO.Path]::GetTempPath()) ('chatpad-task8g-'+[guid]::NewGuid().ToString('N'));$paths=@($ManifestPath,'docs/evidence/one-shot-native-execution-coordinator-task-8g-1.json','tools/ExactInstance/ChatpadOneShotNativeExecutionCoordinator.psm1','tools/ExactInstance/ChatpadOneShotAuthorizationRegistry.cs','tools/Test-ChatpadOneShotNativeExecutionCoordinator.ps1');New-Item -ItemType Directory -Path $temp|Out-Null
    try{
        foreach($relative in $paths){$target=Join-Path $temp $relative;New-Item -ItemType Directory -Path (Split-Path -Parent $target) -Force|Out-Null;Copy-Item -LiteralPath (Join-Path $root $relative) -Destination $target -Force}
        $manifestFile=Join-Path $temp $ManifestPath;$evidenceFile=Join-Path $temp 'docs/evidence/one-shot-native-execution-coordinator-task-8g-1.json';$manifestText=Get-Content -LiteralPath $manifestFile -Raw;$evidenceText=Get-Content -LiteralPath $evidenceFile -Raw;$scalar=Get-ChatpadTask8GExpectedScalars;$scalar.evidence_path='docs/evidence/one-shot-native-execution-coordinator-task-8g-1.json';$scalar.evidence_byte_size=8165;$scalar.evidence_sha256='BB2016A9CFD85BFCA06862E7B3D386EE611355AA2416778BFDCB49C7A6C0BD12';$arrays=Get-ChatpadTask8GExpectedArrays;$cases=[Collections.Generic.List[object]]::new()
        $cases.Add([pscustomobject]@{id='section-removed';expected='section:';mutate={param($m,$e)[void]$m.PSObject.Properties.Remove('one_shot_native_execution_coordinator')}});$cases.Add([pscustomobject]@{id='section-wrong-type';expected='section:';mutate={param($m,$e)$m.one_shot_native_execution_coordinator='tampered'}})
        foreach($pair in $scalar.GetEnumerator()){
            $field=$pair.Key;$expected=$pair.Value
            $cases.Add([pscustomobject]@{id="section-$field";expected="section.$field";mutate=({param($m,$e,$f,$v)$m.one_shot_native_execution_coordinator.$f=if($v-is[bool]){-not$v}elseif($v-is[int]-or$v-is[long]){[long]$v+1}else{'tampered'}}).GetNewClosure();field=$field;value=$expected})
        ...
        foreach($pair in $arrays.GetEnumerator()){
            ...
            $cases.Add([pscustomobject]@{id="$field-property-removed";expected="section.$field";mutate=({param($m,$e,$f)[void]$m.one_shot_native_execution_coordinator.PSObject.Properties.Remove($f)}).GetNewClosure();field=$field})
            $cases.Add([pscustomobject]@{id="$field-added";expected="section.$field";mutate=({param($m,$e,$f)$m.one_shot_native_execution_coordinator.$f=@($m.one_shot_native_execution_coordinator.$f)+@('TASK8G_UNAUTHORIZED_ADDED_ENTRY')}).GetNewClosure();field=$field})
            $cases.Add([pscustomobject]@{id="$field-scalar";expected="section.$field";mutate=({param($m,$e,$f)$m.one_shot_native_execution_coordinator.$f='TASK8G_SCALAR'}).GetNewClosure();field=$field})
            $cases.Add([pscustomobject]@{id="$field-null";expected="section.$field";mutate=({param($m,$e,$f)$m.one_shot_native_execution_coordinator.$f=$null}).GetNewClosure();field=$field})
            $cases.Add([pscustomobject]@{id="$field-manifest-evidence-disagreement";expected="manifest-evidence.$field";mutate=({param($m,$e,$f)$e.$f=@($e.$f)+@('TASK8G_MANIFEST_EVIDENCE_MISMATCH')}).GetNewClosure();field=$field})
            if($expectedValues.Count -gt 0){
                $cases.Add([pscustomobject]@{id="$field-entry-removed";expected="section.$field";mutate=({param($m,$e,$f)$m.one_shot_native_execution_coordinator.$f=@($m.one_shot_native_execution_coordinator.$f|Select-Object -Skip 1)}).GetNewClosure();field=$field})
                $cases.Add([pscustomobject]@{id="$field-changed";expected="section.$field";mutate=({param($m,$e,$f)$v=@($m.one_shot_native_execution_coordinator.$f);$v[0]='TASK8G_CHANGED_ENTRY';$m.one_shot_native_execution_coordinator.$f=$v}).GetNewClosure();field=$field})
                $cases.Add([pscustomobject]@{id="$field-empty";expected="section.$field";mutate=({param($m,$e,$f)$m.one_shot_native_execution_coordinator.$f=@()}).GetNewClosure();field=$field})
            }
            if($expectedValues.Count -gt 1){
                $cases.Add([pscustomobject]@{id="$field-reordered";expected="section.$field";mutate=({param($m,$e,$f)$v=@($m.one_shot_native_execution_coordinator.$f);$x=$v[0];$v[0]=$v[1];$v[1]=$x;$m.one_shot_native_execution_coordinator.$f=$v}).GetNewClosure();field=$field})
                $cases.Add([pscustomobject]@{id="$field-duplicated";expected="section.$field";mutate=({param($m,$e,$f)$v=@($m.one_shot_native_execution_coordinator.$f);$v[1]=$v[0];$m.one_shot_native_execution_coordinator.$f=$v}).GetNewClosure();field=$field})
            }
        }
        $results=[Collections.Generic.List[object]]::new();$canonical=Get-Content -LiteralPath $manifestFile -Raw|ConvertFrom-Json;$pass=Test-ChatpadTask8GManifestEvidence -RepositoryRoot $temp -Manifest $canonical;$results.Add([pscustomobject]@{id='canonical';result=$pass.result;expected='PASS';passed=($pass.result-eq'PASS')})
        foreach($case in $cases){[IO.File]::WriteAllText($manifestFile,$manifestText,[Text.UTF8Encoding]::new($false));[IO.File]::WriteAllText($evidenceFile,$evidenceText,[Text.UTF8Encoding]::new($false));$m=Get-Content -LiteralPath $manifestFile -Raw|ConvertFrom-Json;$e=Get-Content -LiteralPath $evidenceFile -Raw|ConvertFrom-Json;$caseField=Get-ChatpadTask8EValue $case 'field';$caseValue=Get-ChatpadTask8EValue $case 'value';& $case.mutate $m $e $caseField $caseValue;[IO.File]::WriteAllText($manifestFile,($m|ConvertTo-Json -Depth 30)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false));[IO.File]::WriteAllText($evidenceFile,($e|ConvertTo-Json -Depth 30)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false));$out=Test-ChatpadTask8GManifestEvidence -RepositoryRoot $temp -Manifest $m;$matched=@($out.defects|Where-Object{$_-like"$($case.expected)*"}).Count-gt0;$results.Add([pscustomobject]@{id=$case.id;result=$out.result;expected=$case.expected;passed=($out.result-eq'FAIL'-and$matched)})}
        $failed=@($results|Where-Object{-not $_.passed});[pscustomobject]@{result=if($failed.Count){'FAIL'}else{'PASS'};test_count=$results.Count;assertion_count=$results.Count;failed_test_count=$failed.Count;tests=@($results);temporary_data_removed=$true}
    }finally{if(Test-Path -LiteralPath $temp){Remove-Item -LiteralPath $temp -Recurse -Force}}
}

function Test-ChatpadUnsignedPackageManifestEvidence {
    param([Parameter(Mandatory)][string]$RepositoryRoot,[Parameter(Mandatory)][object]$Manifest)
    $defects=[Collections.Generic.List[string]]::new();$section=Get-ChatpadTask8EValue $Manifest 'canonical_unsigned_live_apply_package'
    if($null-eq$section-or$section-is[array]-or$section-is[string]){return[pscustomobject]@{result='FAIL';defect_count=1;defects=@('section: missing-or-wrong-type')}}
    $expected=[ordered]@{
        evidence_path='docs/evidence/canonical-live-apply-package-build-task-8i-p1b-2a.json';evidence_byte_size=18015;evidence_sha256='F7065F15615973926BACD2C6A19C13A71381254BE6E4B1BA4D0B674532CF884E';contract_path='tools/ExactInstance/contracts/chatpad-live-apply-unsigned-package-plan.json';contract_byte_size=7862;contract_sha256='256DCBAE723384A6329677D02E1AEA39EECCE5D4CBE62C7CC8133093B117051B';focused_test_path='tools/Test-ChatpadLiveApplyPackageSourceContract.ps1';focused_test_byte_size=40720;focused_test_sha256='C560129EE2DDC2E6B6C013B003CFDC9906F25388FDA325DD6E71FFC89605D571';schema='chatpad-canonical-live-apply-package-build-evidence-v1';status='LOCAL_DEVELOPMENT_HOST_READINESS_OBSERVED_TESTSIGNING_CONFIGURED_REBOOT_PENDING_HVCI_ACTIVE_PACKAGE_UNSIGNED_UNSTAGED_UNINSTALLED_UNLOADED';readiness='READY_FOR_SELF_TEST_SIGNED_LOCAL_DEVELOPMENT_PACKAGE_PREPARATION_ONLY';source_commit='4510e1f2f8a4b4e31ac40672261cf4ad94ec27c5';driver_project_path='src/driver/ChatpadFilter/ChatpadFilter.vcxproj';driver_project_sha256='5641815C63FBB0C2A7442E555A1B70F3BE4C22014DF2BA29A29E987A2D903112';inf_byte_size=1581;inf_sha256='0200FCF5E1B26F4A594ECEF936DAAEC4B8F2BC5743A9DDAE212AD8F99D05E2FC';sys_byte_size=40960;sys_sha256='E4E7BCA837F6B0C662A24CFDDC260D781FDA85E0B37CAE470D65A9716DB174BB';catalog_byte_size=1202;catalog_sha256='6F0ABF84AE68010008A0360D4DDF716E644DD8D63E669F3AA96F27047EF613FD';accepted_build_count=2;sys_reproducible=$true;architecture='AMD64';sys_signature_state='unsigned';catalog_signature_state='unsigned';deployment_scope='LOCAL_DEVELOPMENT_ONLY';current_required_route='CONTROLLED_LOCAL_DEVELOPMENT_INSTALLATION';production_distribution_signing_required=$false;production_distribution_signing='OUT_OF_SCOPE_OPTIONAL_FUTURE_WORK';production_distribution_objective_classification='OUT_OF_SCOPE_FOR_CURRENT_LOCAL_DEVELOPMENT_OBJECTIVE';current_installability='NOT_INSTALLABLE_UNTIL_SELF_TEST_SIGNED_PACKAGE_EXISTS_AND_CONFIGURED_TESTSIGNING_IS_ACTIVATED_BY_REBOOT';package_signed=$false;package_staged=$false;package_installed=$false;driver_loaded=$false;local_installation_method='NOT_YET_OBSERVED';local_signature_enforcement_state='LOCAL_TEST_SIGNING_CONFIGURED_REBOOT_PENDING';local_candidate_identity='NOT_YET_OBSERVED';local_published_inf_name='NOT_YET_OBSERVED';ordinary_production_signature_enforcement_loadability_claimed=$false;host_readiness_observation_performed=$true;host_readiness_evidence_path='docs/evidence/local-development-host-readiness-observation-task-8i-p1b-ld-o1.json';host_readiness_evidence_sha256='019D59504588384DDD48B90B2315A261AE891030113C7953DF6EA2778F3E651C';secure_boot_state='DISABLED';testsigning_configured=$true;reboot_since_testsigning_change=$false;effective_test_mode_current_boot='NOT_CONCLUSIVELY_OBSERVED';hvci_memory_integrity_state='ACTIVE';embedded_test_signature_required_for_sys=$true;test_certificate_present=$false;blocker='BLOCKED_NATIVE_ADAPTER_SELF_TEST_SIGNED_PACKAGE_NOT_CREATED_AND_CONFIGURED_TESTSIGNING_NOT_EFFECTIVE_UNTIL_REBOOT';next_task='TASK 8I-P1B-LD-S1 — Create and freeze the self-test-signed local-development package';file_signing_count=0;staging_count=0;installation_count=0;native_invocation_count=0;device_query_count=0;binding_count=0;authorization_count=0;windows_mutation_count=0
    }
    foreach($pair in $expected.GetEnumerator()){Test-ChatpadTask8GExpectedScalar -Object $section -Name $pair.Key -Expected $pair.Value -Location 'unsigned-package' -Defects $defects}
    foreach($identity in @(@{path=$expected.evidence_path;size=$expected.evidence_byte_size;hash=$expected.evidence_sha256;prefix='evidence'},@{path=$expected.contract_path;size=$expected.contract_byte_size;hash=$expected.contract_sha256;prefix='contract'},@{path=$expected.focused_test_path;size=$expected.focused_test_byte_size;hash=$expected.focused_test_sha256;prefix='focused-test'})){$file=Join-Path $RepositoryRoot $identity.path;if(-not(Test-Path -LiteralPath $file -PathType Leaf)){$defects.Add("$($identity.prefix): missing");continue};if((Get-Item -LiteralPath $file).Length-ne$identity.size){$defects.Add("$($identity.prefix): byte-size")};if((Get-FileHash -LiteralPath $file -Algorithm SHA256).Hash-cne$identity.hash){$defects.Add("$($identity.prefix): sha256")}}
    try{$evidence=Get-Content -LiteralPath (Join-Path $RepositoryRoot $expected.evidence_path) -Raw|ConvertFrom-Json;$plan=Get-Content -LiteralPath (Join-Path $RepositoryRoot $expected.contract_path) -Raw|ConvertFrom-Json}catch{$defects.Add('json: invalid');return[pscustomobject]@{result='FAIL';defect_count=$defects.Count;defects=@($defects)}}
    foreach($test in @(
        @{n='evidence.schema';a=$evidence.schema;x=$expected.schema},@{n='evidence.status';a=$evidence.status;x='CANONICAL_WINDOWS_11_DRIVER_PACKAGE_BUILT_AND_CATALOGED_UNSIGNED_FOR_LOCAL_DEVELOPMENT_NOT_STAGED_NOT_INSTALLED_NOT_LOADED_NOT_INDEPENDENTLY_AUDITED'},@{n='evidence.readiness';a=$evidence.readiness;x='READY_FOR_LOCAL_DEVELOPMENT_PACKAGE_AND_DEPLOYMENT_CONTRACT_AUDIT_ONLY'},@{n='evidence.source_commit';a=$evidence.source_commit;x=$expected.source_commit},@{n='evidence.project';a=$evidence.driver_project.path;x=$expected.driver_project_path},@{n='evidence.inf';a=$evidence.canonical_inf.sha256;x=$expected.inf_sha256},@{n='evidence.sys';a=$evidence.binary_validation.sha256;x=$expected.sys_sha256},@{n='evidence.cat';a=$evidence.catalog_generation.sha256;x=$expected.catalog_sha256},@{n='evidence.scope';a=$evidence.deployment_policy.deployment_scope;x=$expected.deployment_scope},@{n='evidence.production-classification';a=$evidence.deployment_policy.optional_future_production_distribution.classification;x=$expected.production_distribution_objective_classification},@{n='evidence.production-signing-route';a=$evidence.deployment_policy.optional_future_production_distribution.signing_route;x=$expected.production_distribution_signing},@{n='evidence.installation';a=$evidence.deployment_policy.local_installation_method;x=$expected.local_installation_method},@{n='evidence.enforcement';a=$evidence.deployment_policy.local_signature_enforcement_state;x='NOT_YET_OBSERVED'},@{n='evidence.blocker';a=$evidence.blocker;x='BLOCKED_NATIVE_ADAPTER_LOCAL_DEVELOPMENT_PACKAGE_NOT_INDEPENDENTLY_AUDITED_AND_LOCAL_INSTALLATION_PATH_NOT_OBSERVED'},@{n='evidence.next';a=$evidence.next_task;x='TASK 8I-P1B-LD-R2 — Independent read-only audit of the unsigned canonical package and local-development deployment contract'},@{n='plan.status';a=$plan.status;x=$expected.status},@{n='plan.sys';a=$plan.unsigned_sys.sha256;x=$expected.sys_sha256},@{n='plan.cat';a=$plan.unsigned_catalog.sha256;x=$expected.catalog_sha256},@{n='plan.scope';a=$plan.deployment_policy.deployment_scope;x=$expected.deployment_scope},@{n='plan.production-classification';a=$plan.deployment_policy.optional_future_production_distribution.classification;x=$expected.production_distribution_objective_classification},@{n='plan.production-signing-route';a=$plan.deployment_policy.optional_future_production_distribution.signing_route;x=$expected.production_distribution_signing},@{n='plan.installation';a=$plan.deployment_policy.local_installation_method;x=$expected.local_installation_method},@{n='plan.enforcement';a=$plan.deployment_policy.local_signature_enforcement_state;x=$expected.local_signature_enforcement_state},@{n='plan.installability';a=$plan.current_installability;x=$expected.current_installability},@{n='plan.blocker';a=$plan.blocker;x=$expected.blocker},@{n='plan.next';a=$plan.next_task;x=$expected.next_task}
    )){if($test.a-isnot[string]-or-not$test.a.Equals([string]$test.x,[StringComparison]::Ordinal)){$defects.Add("$($test.n): mismatch")}}
    if(@($evidence.package.inventory).Count-ne3-or@($evidence.catalog_generation.members).Count-ne2-or@($evidence.deployment_policy.local_package_inventory).Count-ne3-or@($plan.package_file_inventory).Count-ne3-or@($plan.future_local_deployment_fields).Count-ne28){$defects.Add('inventory: mismatch')}
    foreach($value in @($evidence.signature_validation.test_certificate_present,$evidence.signature_validation.unexpected_signer_present,$evidence.deployment_policy.package_signed,$evidence.deployment_policy.package_staged,$evidence.deployment_policy.package_installed,$evidence.deployment_policy.driver_loaded,$evidence.deployment_policy.ordinary_production_signature_enforcement_loadability_claimed,$evidence.deployment_policy.host_readiness_observation_performed,$plan.deployment_policy.package_signed,$plan.deployment_policy.package_staged,$plan.deployment_policy.package_installed,$plan.deployment_policy.driver_loaded,$plan.deployment_policy.ordinary_production_signature_enforcement_loadability_claimed,$plan.deployment_policy.optional_future_production_distribution.whcp_hlk_required_for_current_objective,$plan.deployment_policy.optional_future_production_distribution.partner_center_submission_required_for_current_objective,$plan.deployment_policy.optional_future_production_distribution.microsoft_production_signing_required_for_current_objective)){if($value-isnot[bool]-or$value){$defects.Add('prohibited-state: nonzero-or-nonboolean')}}
    [pscustomobject]@{result=if($defects.Count){'FAIL'}else{'PASS'};defect_count=$defects.Count;defects=@($defects)}
}

function Get-ChatpadTask8HExpectedScalars {
    [ordered]@{
        schema='chatpad-live-execution-authorization-gate-evidence-v1';status='EXPLICIT_ONE_SHOT_LIVE_AUTHORIZATION_GATE_SOURCE_PRESENT_RECORDING_ONLY_TESTED_PRODUCTION_PROVIDER_NOT_CONSTRUCTED_NO_NATIVE_EXECUTION_NO_BINDING_NO_MUTATION_LIVE_EXECUTION_STILL_UNAUTHORIZED_PENDING_INDEPENDENT_AUDIT';readiness='READY_FOR_INDEPENDENT_LIVE_AUTHORIZATION_GATE_AUDIT_ONLY';created_from_task='TASK 8H-1';remediated_by_task='TASK 8H-1R1';remediation_parent_commit='09711c59f272dfaf275b7f7442c607c9c148d34a';remediation_status='LIVE_AUTHORIZATION_GATE_SOURCE_INTEGRITY_AND_LITERAL_ACKNOWLEDGEMENT_REMEDIATED_RECORDING_ONLY_NO_NATIVE_EXECUTION_NO_BINDING_NO_MUTATION';implementation_branch='feature/native-adapter-live-execution-authorization-gate';implementation_parent_commit='79ec174dabd7e73f2d01021168921021bc118702';implementation_commit_subject='feat: implement explicit live execution authorization gate';implementation_commit='DERIVED_FROM_GIT_AFTER_COMMIT';gate_source_path='tools/ExactInstance/ChatpadLiveExecutionAuthorizationGate.psm1';gate_source_byte_size=20196;gate_source_sha256='2EC06735E8397A4562A1538F763C914C62B9F933DE6624333F053BD4165EE42E';managed_helper_path='tools/ExactInstance/ChatpadLiveAuthorizationRegistry.cs';managed_helper_byte_size=1627;managed_helper_sha256='86BE5EF2D1A7211A48B60B05F6AF66750CA952F1FAA46FBCF6DFECFDF9F5A687';focused_test_path='tools/Test-ChatpadLiveExecutionAuthorizationGate.ps1';focused_test_byte_size=26935;focused_test_sha256='50D3910597091B6888401DCAD113BC5A67E751844F09BA2959F509992EDC33AA';managed_authorization_type='Chatpad.LiveAuthorization.LiveNativeApplyAuthorization';accepted_operation='APPLY';shared_container_id='{828F4587-006F-5AD1-B169-6AF57905DFDE}';operator_confirmation_id='operator-confirmation-c445630fbb303c7c';live_authorization_phrase='AUTHORIZE_ONE_LIVE_NATIVE_APPLY_ATTEMPT';task_8e_evidence_path='docs/evidence/native-adapter-nonexecuting-implementation-task-8e-1.json';task_8e_evidence_sha256='493C80C445B9D3903D70509AA704177396172B139CCDD037195D4FA84AA9FB6F';task_8f_evidence_path='docs/evidence/native-adapter-production-backend-source-task-8f-1.json';task_8f_evidence_sha256='93B0D93E63FA43A9D5C7EE647BD5E261085CD3A4F47F5CD4FD4D1DFA01800042';task_8g_evidence_path='docs/evidence/one-shot-native-execution-coordinator-task-8g-1.json';task_8g_evidence_sha256='BB2016A9CFD85BFCA06862E7B3D386EE611355AA2416778BFDCB49C7A6C0BD12';task_8g_audited_implementation_commit='79ec174dabd7e73f2d01021168921021bc118702';runtime_current_file_hashing_enabled=$true;caller_map_compared_to_accepted_map=$true;current_file_map_compared_to_accepted_map=$true;caller_map_compared_to_current_file_map=$true;critical_source_hash_key_set_must_match_fixed_inventory=$true;literal_boolean_acknowledgements_required=$true;single_item_array_acknowledgements_rejected=$true;string_boolean_acknowledgements_rejected=$true;source_integrity_fixture_tampering_rejected=$true;source_integrity_missing_file_rejected=$true;source_integrity_directory_rejected=$true;source_integrity_path_traversal_rejected=$true;source_integrity_path_semantics='REPOSITORY_RELATIVE_REGULAR_FILE_NO_REPARSEPOINT_NO_TRAVERSAL';root_of_trust_classification='DEPENDENT_SOURCE_FILES_RUNTIME_VALIDATED_GATE_SOURCE_EXTERNALLY_VALIDATED';atomic_mechanism='CONDITIONAL_WEAK_TABLE_WITH_INTERLOCKED_COMPARE_EXCHANGE_LIVE_AUTHORIZATION_RECORD';recording_authorization_type='Chatpad.OneShotAuthorization.Registry recording capability';live_gate_authorization_type='Chatpad.LiveAuthorization.LiveNativeApplyAuthorization';production_execution_state='UNAVAILABLE_IN_TASK_8H_1';recording_live_type_confusion_rejected=$true;live_authorization_rejected_by_recording_coordinator=$true;concurrency_test_uses_actual_registry=$true;concurrent_contender_count=2;concurrent_successful_consumption_count=1;concurrent_replay_rejection_count=1;concurrent_production_construction_count=0;concurrent_native_invocation_count=0;bounded_race_iteration_count=16;replay_after_success_rejected=$true;replay_after_fake_consumer_failure_rejected=$true;replay_after_exception_rejected=$true;replay_after_cleanup_failure_rejected=$true;request_transfer_rejected=$true;wrapper_authorization_rejected=$true;copy_reconstruction_rejected=$true;deserialized_authorization_rejected=$true;forged_type_authorization_rejected=$true;string_authorization_rejected=$true;boolean_authorization_rejected=$true;import_produces_authorization=$false;production_provider_registered=$false;production_provider_selected=$false;production_provider_constructed=$false;production_provider_loaded=$false;production_provider_invoked=$false;production_provider_available_to_gate=$false;production_provider_object_accepted_by_gate=$false;production_provider_registration_count=0;production_provider_selection_count=0;production_provider_construction_count=0;production_provider_load_count=0;production_provider_invocation_count=0;device_query_count=0;native_invocation_count=0;setupapi_newdev_invocation_count=0;binding_count=0;windows_mutation_count=0;driver_action_count=0;rollback_count=0;restore_count=0;artifact_compile_output_access_count=0;powershell_7_result='PASS';powershell_7_test_count=75;powershell_7_assertion_count=2146;windows_powershell_result='PASS';windows_powershell_test_count=75;windows_powershell_assertion_count=2146;remaining_blocker='BLOCKED_NATIVE_ADAPTER_LIVE_EXECUTION_AUTHORIZATION_GATE_NOT_INDEPENDENTLY_AUDITED';next_task='TASK 8H-2 — Independent read-only audit of the explicit one-shot live-execution authorization gate';live_execution_still_unauthorized=$true;live_invocation_performed=$false;native_execution_performed=$false;binding_performed=$false;windows_mutation_performed=$false;ready_for_native_execution=$false
    }
}

function Get-ChatpadTask8HExpectedArrays {
    [ordered]@{
        public_surface=@('New-ChatpadOneShotLiveNativeApplyAuthorization');private_gate_surface=@('Get-LiveExecutionAuthorizationGateConstants','Test-LiveExecutionAuthorizationGateRequest','Get-LiveGateRequestFingerprint','Invoke-TestOnlyLiveGateRecordingConsumer');managed_authorization_helper_surface=@('Chatpad.LiveAuthorization.LiveAuthorizationRegistry.CreateOneShotLiveNativeApplyAuthorization','Chatpad.LiveAuthorization.LiveAuthorizationRegistry.TryConsumeOneShotLiveNativeApplyAuthorization');managed_method_signatures=@('Chatpad.LiveAuthorization.LiveNativeApplyAuthorization Chatpad.LiveAuthorization.LiveAuthorizationRegistry.CreateOneShotLiveNativeApplyAuthorization(System.String fingerprint)','System.Boolean Chatpad.LiveAuthorization.LiveAuthorizationRegistry.TryConsumeOneShotLiveNativeApplyAuthorization(Chatpad.LiveAuthorization.LiveNativeApplyAuthorization authorization, System.String fingerprint)');managed_constructor_signatures=@();required_authorization_input_array=@('operation','ordered_target_chain','container_id','operator_confirmation_identity','live_authorization_phrase','task_8e_evidence_identity','task_8f_evidence_identity','task_8g_evidence_identity','task_8g_audited_implementation_commit','critical_source_hashes','rollback_recovery_reviewed','one_shot_attempt_understood');accepted_ordered_target_chain=@('USB\VID_045E&PID_028E\1C21F10','USB\VID_045E&PID_028E&IG_00\8&2AF61D70&1&00','HID\VID_045E&PID_028E&IG_00\9&2E72F677&0&0000');authorization_object_properties=@('process_bound','reference_identity_bound','one_attempt_only','non_serializable_as_usable_authority','non_transferable_to_another_request','non_transferable_to_another_provider','bound_to_exact_evidence_source_fingerprint','consumed_before_provider_construction_or_invocation','permanently_consumed_after_success_rejection_exception_or_cleanup_failure','no_reset_or_retry_method','no_exported_registration_or_provider_binding_method','no_arbitrary_payload_or_provider_parameter');fixed_externally_validated_root_of_trust_inventory=@('tools/ExactInstance/ChatpadLiveExecutionAuthorizationGate.psm1','tools/ExactInstance/ChatpadLiveAuthorizationRegistry.cs')
    }
}
...
function Test-ChatpadTask8HManifestEvidence {
    ...
    if($null-eq$section-or$section-is[array]-or$section-is[string]){return [pscustomobject]@{result='FAIL';defect_count=1;defects=@('section: missing-or-wrong-type')}}
    $expected=Get-ChatpadTask8HExpectedScalars;$arrays=Get-ChatpadTask8HExpectedArrays;$identity=[ordered]@{evidence_path='docs/evidence/live-execution-authorization-gate-task-8h-1.json';evidence_byte_size=9278;evidence_sha256='2EAC8D3C707C53F77859620DC9D7D43903DC61F334774898BBFD56944977DBF2'}
    ...
    foreach($fileIdentity in @(@{path='tools/ExactInstance/ChatpadLiveExecutionAuthorizationGate.psm1';size=20196;hash='2EC06735E8397A4562A1538F763C914C62B9F933DE6624333F053BD4165EE42E';prefix='gate_source'},@{path='tools/ExactInstance/ChatpadLiveAuthorizationRegistry.cs';size=1627;hash='86BE5EF2D1A7211A48B60B05F6AF66750CA952F1FAA46FBCF6DFECFDF9F5A687';prefix='managed_helper'},@{path='tools/Test-ChatpadLiveExecutionAuthorizationGate.ps1';size=26935;hash='50D3910597091B6888401DCAD113BC5A67E751844F09BA2959F509992EDC33AA';prefix='focused_test'})){
    ...
    if((Get-Item -LiteralPath $evidencePath).Length-ne9278){$defects.Add('evidence.byte_size: mismatch')};if((Get-FileHash -LiteralPath $evidencePath -Algorithm SHA256).Hash-cne'2EAC8D3C707C53F77859620DC9D7D43903DC61F334774898BBFD56944977DBF2'){$defects.Add('evidence.sha256: mismatch')}
    try{$evidence=Get-Content -LiteralPath $evidencePath -Raw|ConvertFrom-Json}catch{$defects.Add('evidence.json: invalid');return [pscustomobject]@{result='FAIL';defect_count=$defects.Count;defects=@($defects)}}
    foreach($pair in $expected.GetEnumerator()){Test-ChatpadTask8GExpectedScalar -Object $evidence -Name $pair.Key -Expected $pair.Value -Location 'evidence' -Defects $defects;Test-ChatpadTask8GExpectedScalar -Object $section -Name $pair.Key -Expected (Get-ChatpadTask8EValue $evidence $pair.Key) -Location 'manifest-evidence' -Defects $defects}
    foreach($pair in $arrays.GetEnumerator()){
        foreach($defect in (Get-ChatpadExactOrdinalArrayPropertyDefects -Object $evidence -PropertyName $pair.Key -ExpectedValues $pair.Value -FieldName "evidence.$($pair.Key)").defects){$defects.Add($defect)}
...
function Invoke-ChatpadTask8HRegression {
    ...
    try{
        foreach($relative in $paths){$target=Join-Path $temp $relative;New-Item -ItemType Directory -Path (Split-Path -Parent $target) -Force|Out-Null;Copy-Item -LiteralPath (Join-Path $root $relative) -Destination $target -Force}
        $manifestFile=Join-Path $temp $ManifestPath;$evidenceFile=Join-Path $temp 'docs/evidence/live-execution-authorization-gate-task-8h-1.json';$manifestText=Get-Content -LiteralPath $manifestFile -Raw;$evidenceText=Get-Content -LiteralPath $evidenceFile -Raw;$scalar=Get-ChatpadTask8HExpectedScalars;$scalar.evidence_path='docs/evidence/live-execution-authorization-gate-task-8h-1.json';$scalar.evidence_byte_size=9278;$scalar.evidence_sha256='2EAC8D3C707C53F77859620DC9D7D43903DC61F334774898BBFD56944977DBF2';$arrays=Get-ChatpadTask8HExpectedArrays;$cases=[Collections.Generic.List[object]]::new()
...
if($RunTask8FR1Regression){
    $regression=Invoke-ChatpadTask8FR1Regression -ManifestPath $ManifestPath
    $regression|ConvertTo-Json -Depth 8
    if($regression.result-ne'PASS'){exit 1}
    exit 0
}
if($RunTask8GRegression){
    $regression=Invoke-ChatpadTask8GRegression -ManifestPath $ManifestPath
    $regression|ConvertTo-Json -Depth 8
    if($regression.result-ne'PASS'){exit 1}
    exit 0
}
...
if($null-eq$hostSection-or-not(Test-Path -LiteralPath $hostEvidencePath -PathType Leaf)){$defects.host_readiness++}else{
    try{$hostEvidence=Get-Content -LiteralPath $hostEvidencePath -Raw|ConvertFrom-Json}catch{$hostEvidence=$null;$defects.host_readiness++}
    if($hostSection.evidence_path-cne'docs/evidence/local-development-host-readiness-observation-task-8i-p1b-ld-o1.json'-or[long]$hostSection.evidence_byte_size-ne8467-or$hostSection.evidence_sha256-cne'019D59504588384DDD48B90B2315A261AE891030113C7953DF6EA2778F3E651C'-or$hostSection.schema-cne'chatpad-local-development-host-readiness-observation-v1'-or$hostSection.status-cne'LOCAL_DEVELOPMENT_HOST_READINESS_OBSERVED_TESTSIGNING_CONFIGURED_REBOOT_PENDING_HVCI_ACTIVE_PACKAGE_UNSIGNED_UNSTAGED_UNINSTALLED_UNLOADED'-or$hostSection.readiness-cne'READY_FOR_SELF_TEST_SIGNED_LOCAL_DEVELOPMENT_PACKAGE_PREPARATION_ONLY'-or$hostSection.selected_outcome-cne'LOCAL_TEST_SIGNING_CONFIGURED_REBOOT_PENDING'-or$hostSection.secure_boot_state-cne'DISABLED'-or$hostSection.bitlocker_protection_status-cne'Off'-or$hostSection.system_volume_lock_status-cne'Unlocked'-or-not[bool]$hostSection.testsigning_configured-or[bool]$hostSection.reboot_since_testsigning_change-or$hostSection.effective_test_mode_current_boot-cne'NOT_CONCLUSIVELY_OBSERVED'-or$hostSection.hvci_memory_integrity_state-cne'ACTIVE'-or-not[bool]$hostSection.embedded_test_signature_required_for_sys-or[bool]$hostSection.package_signed-or[bool]$hostSection.package_staged-or[bool]$hostSection.package_installed-or[bool]$hostSection.driver_loaded-or[long]$hostSection.observer_mutation_count-ne0-or[long]$hostSection.external_bcd_mutation_count-ne1-or[long]$hostSection.overall_lane_mutation_count-ne1-or$hostSection.supersedes_observation_sha256-cne'255C874A774CF6492B4A70A99214988824351E738B9AC8D10443F59B441E31C3'-or$hostSection.superseded_outcome-cne'LOCAL_DEVELOPMENT_SIGNING_ROUTE_NOT_YET_DETERMINABLE'-or$hostSection.supersession_reason-cne'TARGETED_HOST_READINESS_OBSERVATION_REMEDIATION'-or$hostSection.blocker-cne'BLOCKED_NATIVE_ADAPTER_SELF_TEST_SIGNED_PACKAGE_NOT_CREATED_AND_CONFIGURED_TESTSIGNING_NOT_EFFECTIVE_UNTIL_REBOOT'-or$hostSection.next_task-notmatch'^TASK 8I-P1B-LD-S1 .+ Create and freeze the self-test-signed local-development package$'){$defects.host_readiness++}
    if($null-ne$hostEvidence-and($hostEvidence.selected_outcome-cne$hostSection.selected_outcome-or$hostEvidence.secure_boot.state-cne'DISABLED'-or$hostEvidence.bitlocker.protection_status-cne'Off'-or$hostEvidence.pre_observation_context.effective_test_mode_current_boot-cne'NOT_CONCLUSIVELY_OBSERVED'-or-not[bool]$hostEvidence.hvci.running-or[long]$hostEvidence.observer_mutation_count-ne0-or[long]$hostEvidence.overall_lane_mutation_count-ne1)){$defects.host_readiness++}
}
...
foreach($entry in $entries){
    ...
    if($null-eq$policyProperty-or$policyProperty.Value-notin@('canonical_lf_text','raw_file_bytes')-or$null-eq$classificationProperty-or$null-eq$commitProperty-or[string]$commitProperty.Value-notmatch'^[0-9a-f]{40}$'){
        ...
        $approvedRawLiveObservation=$classification-eq'text'-and$policy-eq'raw_file_bytes'-and$entry.evidence_classification-eq'live_readonly_host_observation'
        if(($classification-eq'text'-and$policy-ne'canonical_lf_text'-and-not$approvedRawLiveObservation)-or($classification-eq'binary'-and$policy-ne'raw_file_bytes')-or($classification-notin@('text','binary'))){$defects.hash_policy++}
        else {
            try {
    ...
    if($entry.evidence_classification-notin@('synthetic','live_readonly_host_observation')){$defects.declared_result++}
}
...
$task8FR1Validation=Test-ChatpadTask8FR1ManifestEvidence -RepositoryRoot $root -Manifest $manifest
$defects.task_8f_r1=[int]$task8FR1Validation.defect_count
...
if($inventoryDefectCount -or $inventoryCounts.tracked_ps1_count -ne 51 -or $inventoryCounts.tracked_psm1_count -ne 11 -or$inventoryCounts.tracked_powershell_count-ne62-or$inventoryCounts.parsed_ps1_count-ne51-or$inventoryCounts.parsed_psm1_count-ne11-or$inventoryCounts.parsed_powershell_count-ne62-or$inventoryCounts.parse_error_count-ne0-or$inventoryCounts.duplicate_normalized_path_count-ne0-or$inventoryCounts.missing_count-ne0-or$inventoryCounts.extra_count-ne0){$defects.powershell_inventory++}
$analyzer=$manifest.readiness.psscriptanalyzer
if($manifest.readiness.psscriptanalyzer_status-eq'PASS'){
    if($null-eq$analyzer-or[int]$analyzer.analyzed_file_count-ne58-or[int]$analyzer.error_count-ne0-or[int]$analyzer.tool_failure_count-ne0-or[bool]$analyzer.blanket_suppression_used){$defects.psscriptanalyzer++}
